...
export function sessionState(sessionId) {
  ...
  if (!s) {
    s = {
      ...
      // —— D-30（FR-T5）：待批令牌绑定的设计文档集指纹 ——
      // pendingDocHash = 历轮 documents **并集**的指纹（doc-hash.mjs 计算：路径 + 内容双绑）；
      // pendingDocPaths = 同一并集的归一（resolve + "/" 归一）并排序后的绝对路径表——续期时按它重算。
      // 空文档集或任一文档读不到 → pendingDocHash = null → 续期一律拒绝（决策 D6/N3 fail-closed）。
      // 二者与 designToken/pendingDesignToken 同域：**绝不进持久化视图**（viewOfSessionState 白名单
      // 之外；session-store.normalizeRestored 会丢弃）——与 N4/T7「凭证不进 session-state 镜像」同契约。
      pendingDocHash: null,
      pendingDocPaths: [],
      // —— advisor 会话级覆盖（docs/2026-09-01-advisor-config-design.md §3.6）——
      // null = 无覆盖；结构 { round1?: {provider?,model?,effort?,timeoutMs?,runner?},
      // convergence?: {…同}, includeProjectGuide?: boolean }。优先级高于全局组配置
      // （字段级浅合并，见 mergeAdvisorGroup），经 advisor_config 工具读写，会话销毁即失效。
      // runner = R4 §6.2（D-13）三面同步后入白名单（字符串简写 "dsh"/"codex-cli" 或归一化
      // runner 对象，校验走 advisor.mjs normalizeRunnerValue）。
