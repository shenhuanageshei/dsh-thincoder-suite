...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		/** 推理档 effort：目录含该模型的 reasoningEfforts 时按模型档位（null 档不进下拉），否则全局枚举。 */
		function EffortInput(props) {
			...
			return h("select", {
				...
				h("option", { key: "", value: "" }, efforts ? "（不设——模型默认）" : "（不设——适配器默认）"),
				list.map(function (v) {
		...
		/** runner 类型选择（一期 codex-runner）：未设/dsh/codex-cli。 */
		...
		/** codex 模型下拉（GET /codex/models 动态发现；失败降级手输）。 */
		...
		/** codex effort 下拉：随所选模型联动（目录可用时），否则手输（透传）。 */
		...
		function ThincoderPage(props) {
			...
			useEffect(function () {
				var cancelled = false;
				function finishRpc(fallbackNote) {
					if (cancelled) return;
					loadCatalogViaRpc(function (cat2) {
						if (cancelled) return;
						if (cat2 && cat2.ok) setCatalog(cat2);
						else setCatalog({ ok: false, note: fallbackNote + "；" + ((cat2 && cat2.note) || "RPC 亦不可用") });
					});
				}
				apiCall("GET", "/catalog").then(function (d) {
					if (cancelled) return;
					if (d && d.ok === true && Array.isArray(d.providers) && d.providers.length > 0) {
						var byGroup = {};
						...
						setCatalog({ ok: true, providers: d.providers, byGroup: byGroup, source: "settings" });
						return;
					}
					finishRpc((d && d.error) || "catalog 端点无数据");
				}).catch(function (e) {
					finishRpc(String(e && e.message ? e.message : e));
				});
			...
			function applySession() {
				...
				GROUP_KEYS.forEach(function (gk) {
					var g = draft.advisor[gk] || {};
					// 一期 codex-runner：会话覆盖（sanitizeSessionAdvisor）不收 runner 字段——
					// codex 组跳过 apply（runner 属全局层，避免把 codex 模型误写进 provider/model）
			...
			function groupCard(gk) {
				...
				var gridChildren = isCodex ? [
					label("codex 模型（动态目录）", h(CodexModelInput, {
						catalog: codexModels, value: g.codexModel,
						onChange: function (v) { setGroupField(gk, "codexModel", v); },
					})),
					label("codex effort（随模型联动）", h(CodexEffortInput, {
						catalog: codexModels, model: g.codexModel, value: g.codexEffort,
				...
				return h("div", { className: "tc-card", key: "pool" }, [
					...
					rows.map(function (row, i) {
						...
						return h("div", { className: "tc-row pool", key: "row" + i }, [
							h(RunnerSelect, {
								value: row.runnerKind || "",
								onChange: function (v) { setPoolRow(i, "runnerKind", v); },
							}),
							rowCodex
								? h(CodexModelInput, {
									catalog: codexModels, value: row.codexModel,
							...
							rowCodex
								? h(CodexEffortInput, {
									catalog: codexModels, model: row.codexModel, value: row.codexEffort,
				...
				return h("div", { className: "tc-card", key: "codex" }, [
					...
					h("div", { className: "tc-grid" }, [
						...
						label("codex 默认模型（行未选模型时用）", h(CodexModelInput, {
							catalog: codexModels, value: cc.model || "",
