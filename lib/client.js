...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 0.9.3：consult 看门狗（consultTimeoutMs）合法区间（与后端 PUT 校验同规则）
		var CONSULT_TIMEOUT_MIN = 60000;
		...
		function validateDraft(draft) {
			...
			// 0.9.3：consult 看门狗超时（与后端 PUT 校验同规则：60000..3600000 整数）
			if (draft.consultTimeoutMs !== "" && draft.consultTimeoutMs !== undefined && draft.consultTimeoutMs !== null) {
				var cot = Number(draft.consultTimeoutMs);
				if (!Number.isFinite(cot) || cot < CONSULT_TIMEOUT_MIN || cot > CONSULT_TIMEOUT_MAX || !Number.isInteger(cot)) {
		...
		function draftToPayload(draft) {
			...
			// 0.9.3：consult 看门狗超时（空字段不发送 = 不覆盖 base/内置缺省）
			if (draft.consultTimeoutMs !== "" && draft.consultTimeoutMs !== undefined && draft.consultTimeoutMs !== null) {
		...
		function effectiveToDraft(effective) {
			...
			return {
				...
				// 0.9.3：consult 看门狗超时（顶层全局字段——effective 数字 → 表单串）
		...
		function mergeDraftPreservingTouched(seeded, cur, touched) {
			...
			if (touched["consultTimeoutMs"]) next.consultTimeoutMs = curOf(["consultTimeoutMs"], ""); // 0.9.3：busy 窗口内的编辑保留
		...
		function ThincoderPage(props) {
			...
			// 0.9.3：consult 看门狗超时——当前生效 + 来源标注（U2 同款）
			var effCt = view.effective.consultTimeoutMs;
			var ctSource = fieldSource(base, user, ["consultTimeoutMs"]);
			...
			return h("div", { className: "tc-page" }, [
				...
				consultPoolCard(),
				h("div", { className: "tc-card", key: "consultto" }, [
					h("div", { className: "tc-card-title" }, "会诊看门狗（consultTimeoutMs）"),
					h("div", { className: "tc-grid" }, [
						label("会诊子代理看门狗超时（毫秒）", h("input", {
							className: "tc-field", type: "number", min: CONSULT_TIMEOUT_MIN, max: CONSULT_TIMEOUT_MAX, step: 1000,
					...
					h("div", { className: "tc-hint" }, "dsh/codex 会诊子代理共用的看门狗预算：到点中止并记失败（timed out after Nmin）。慢模型（如 deepseek-v4-pro + max）在超大会话下可能超过默认 10 分钟——按需调大；保存即生效。当前生效："
						+ (effCt === undefined ? "600000（默认）" : effCt) + "（" + ctSource + "）· 合法区间 " + CONSULT_TIMEOUT_MIN + "~" + CONSULT_TIMEOUT_MAX + "。"),
				]),
				codexCliCard(),
				engCard(),
