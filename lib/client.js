...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 批 7（D-P4/D-P15）：两个「项目属性」声明键（advisor.standardsDoc / advisor.documentMapDoc）。
		// 值是**字符串文档路径**，无「合法区间」语义 ⇒ **不加** min/max 常量（对齐既有无区间
		// 标量先例 engCoderMaxTokens）。
		var PROJECT_DOC_KEYS = ["standardsDoc", "documentMapDoc"];
		...
		function validateDraft(draft) {
			...
			// 批 7（D-P4/J11）：两个声明键是**字符串文档路径**（无区间——D-P15）。
		...
		function draftToPayload(draft) {
			...
			// 批 7（J11/AC-P12c）：两个声明键**一律照发**——空串 `""` 必须**送出**（服务端据此
			// **删除该键** = 撤销声明；「空字段不发送」会把用户锁死在已声明态）。
		...
		function effectiveToDraft(effective) {
			...
			return {
				advisor: {
					...
					// 批 7（D-P4）：两个声明键（字符串路径；缺省 = 未声明 → 注入器走显式降级句）
		...
		function mergeDraftPreservingTouched(seeded, cur, touched) {
			...
			// 批 7（D-P4/J11）：两个声明键——清空 = 撤销声明（下一次保存把 "" 送出 ⇒ 服务端删键）
			PROJECT_DOC_KEYS.forEach(function (k) {
		...
		function ThincoderPage(props) {
			...
			// 批 7（D-P4）：两个声明键的当前生效回显（仅配置层，复用 fieldSource；U2 来源标注）
			...
			return h("div", { className: "tc-page" }, [
				...
				// 批 7（D-P4/D-P8/J8/J11）：两个「项目属性」声明键——一张卡片两个文本框。
				// 键 = advisor.standardsDoc（项目标准文档）· advisor.documentMapDoc（文档地图）。
				// 两者都是字符串文档路径（相对会话 cwd；无合法区间 ⇒ 无 min/max，D-P15）。
				...
				h("div", { className: "tc-card", key: "projectdocs" }, [
					h("div", { className: "tc-card-title" }, "项目文档声明（standardsDoc / documentMapDoc）"),
					h("div", { className: "tc-hint" }, "这两个键把「评审按什么判」交给项目自己声明（不再写死任何文件名，故插件可移植）。留空/清空 = 未声明：注入器走显式降级句（不是静默跳过）。路径必须是文档文件（服务端用与写门禁同一个判据 isDocPath 预检）；是否可读在评审时判——声明了却读不到会响亮点名该配置键。"),
					h("div", { className: "tc-grid" }, [
