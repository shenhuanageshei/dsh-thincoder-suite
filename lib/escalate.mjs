...
// 指向 eng_coder ③ dsh 子代理路径（DP-1：R2 方案 A budgetCap 内部截止 = background=false/
// 缺省的同步快路径〔受平台墙钟约束，到点 abort + 响亮告警〕；R5 §7.2 方案 B 终裁——
// 显式 background=true → 派后台 job〔kind escalate-dsh，免墙钟，dshBackgroundTimeoutMs
// 兜底防挂死〕；同步快路径与钳制按设计保留）；codex 路径（T2.1）为子进程，带
 ...
 * @param background — R5 §7.2：dsh 行 true = 派后台 job（kind escalate-dsh，免平台墙钟，
 *        dshBackgroundTimeoutMs 兜底防挂死）；缺省/false = 现状同步快路径（budgetCap 内部
 *        截止）。codex 行/followup 同参数语义（强制后台派发，无视预算）。
...
export async function runEscalate(deps, task, model, followup, background) {
  ...
  if (background === true) {
    ...
    if (jobs && typeof jobs.start === "function") {
      ...
      try {
        ...
        console.warn("[thincoder-suite] ctx.jobs 派发失败（" + (e?.message ?? String(e))
          + "）——escalate background=true 回落同步执行（budgetCap 内部截止）")
      }
    } else {
      // jobs / ctx.subagents 缺失 → 同步降级（budgetCap 内部截止，响亮告警——绝不无声）
      console.warn("[thincoder-suite] escalate background=true 不可用（"
  ...
  // 墙钟下超限会被静默截断且无术后报告）。到点 abort + 响亮告警。background=false/缺省 =
  // 同步快路径（R5 §7.2 保留，短任务体验不变）；长任务请走 background=true 或 codex runner。
  ...
  try {
    ...
    if (signal?.aborted || e?.name === "AbortError") {
      ...
      return deathLine(annotateAbort(new Error("escalate (" + tag + ") aborted."), src, layer, "dsh subagent abort"), src)
    }
