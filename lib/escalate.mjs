            ...
            const done = (async () => {
              ...
              try {
                if (!ctx?.subagents || typeof ctx.subagents.start !== "function") {
                  return jobOutcome(handle, { status: "failed", detail: "ctx.subagents unavailable", output: "escalate (" + tag + ") failed: ctx.subagents is unavailable inside background job (cannot recover after dispatch)" })
                }
                ...
                return jobOutcome(handle, { status: "failed", detail: String(e?.message ?? e), output: "escalate (" + tag + ") failed to start: " + (e?.message ?? String(e)) })
              } finally {
