...
import { normalizeRunnerValue, runCodexTask, codexRowLabel, CODEX_TASK_PREAMBLE } from "./codex-adapter.mjs"
...
export async function runEscalate(deps, task, model, followup) {
  ...
  if (nr.ok && nr.runner.kind === "codex-cli") {
    ...
    try {
      ...
      codexThreads.delete(threadKey)
      const partial = env.text ? "\nPartial output:\n" + env.text.slice(0, 2000) : ""
      return "escalate (" + tag + ") ended: codex-cli " + env.code + " — " + (env.userMessage || env.diagnostics || "unknown") + partial + capNote
    } catch (e) {
      codexThreads.delete(threadKey)
