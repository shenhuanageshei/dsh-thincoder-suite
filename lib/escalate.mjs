          ...
          run: () => {
            ...
            if (Number.isFinite(backstopMs) && backstopMs > 0) {
              backstopTimer = setTimeout(() => {
                ...
                try { ctrl.abort(new Error("escalate dsh background backstop deadline reached")) } catch { /* already aborted */ }
              }, backstopMs)
              backstopTimer.unref?.()
            }
            ...
            const done = (async () => {
              ...
              try {
                ...
                if (ctrl.signal.aborted || e?.name === "AbortError") {
                  return { status: "failed", detail: "aborted", output: "escalate (" + tag + ") aborted." }
                }
