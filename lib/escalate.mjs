...
// 指向 eng_coder ③ dsh 子代理路径（批 21 / FR-1② 三态：省略 ⇒ 派后台 job〔kind
// escalate-dsh，免墙钟，dshBackgroundTimeoutMs 兜底防挂死〕——默认后台，与 eng_coder dsh
// 路径同款；显式 background=false ⇒ DP-1 同步快路径〔R2 方案 A budgetCap 内部截止，受
// 平台墙钟约束，到点 abort + 响亮告警〕；jobs 缺失/派发失败 ⇒ 告警〔批 21 FR-3 双通道：
// console.warn + 随工具返回文本可见〕+ 回落同步）；codex 路径（T2.1）为子进程，带
...
// R4 收尾微修复轮（code review 跟进 #2）：本文件不直接消费 sessionState（state 经
// deps.state 注入——runEscalate 的 { ctx, agent, config, state, signal } 解构），死 import 移除。
import { engEffective } from "./state.mjs"
...
import { resolveCodexBudgetCapMs, getJobsService, jobsDispatchReply, codexJobsDispatchReply, checkInFlightJob, setInFlightJob, clearInFlightJob, sessionStateViewWithGeneration, resolveDshBackgroundTimeoutMs, PLATFORM_RUN_CODE_WALL_MS } from "./advisor.mjs"
 ...
 * 同链——不再把 runner.timeoutMs 原样透传绕过 budgetCap）；预算 > budgetCap 或显式
 * background=true（R5 §7.2：followup 续轮往往更长，强制后台）且 ctx.jobs 可用 → jobs
 * 派发（run() 内全预算 + 自有 watchdog；cancel 走 ctrl.abort）；否则（≤ cap / jobs 缺失/
 * 派发抛错）→ 同步执行（> cap 时钳制 budgetCap + 响亮告警）。D-20：簿记在 done 成功
 * 分支恰好一次。
 * @param deps — runEscalate 的 deps（ctx/agent/config/state/signal + spawn 注入缝）
 * @param p { cliCfg, tag, runner, sandbox, taskText, resumeThreadId, effNote, background? }
 */
async function runEscalateCodex(deps, p) {
  ...
  const background = p.background === true // R5 §7.2：强制后台（无视预算，长续轮显式请求）
  ...
  const validDefTimeout = Number.isInteger(cliCfg.defaultTimeoutMs) && cliCfg.defaultTimeoutMs >= 30000 && cliCfg.defaultTimeoutMs <= 3600000
  const timeoutWarnings = []
  if (!validDefTimeout && cliCfg.defaultTimeoutMs !== undefined && cliCfg.defaultTimeoutMs !== null) {
    timeoutWarnings.push("codexCli.defaultTimeoutMs 非法，回落 600000ms")
  }
  const defTimeout = validDefTimeout ? cliCfg.defaultTimeoutMs : 600000
  ...
  const runnerIdle = idleOf(runner.idleTimeoutMs)
  const configIdle = idleOf(cliCfg.idleTimeoutMs)
  if (runner.idleTimeoutMs !== undefined && runnerIdle === null) timeoutWarnings.push("runner.idleTimeoutMs 非法，回落 300000ms")
  if (runnerIdle === null && cliCfg.idleTimeoutMs !== undefined && configIdle === null) timeoutWarnings.push("codexCli.idleTimeoutMs 非法，回落 300000ms")
  const idleTimeoutMs = runnerIdle ?? configIdle ?? 300000
  const timeoutNote = timeoutWarnings.length > 0 ? "\n\n[thincoder-suite] " + timeoutWarnings.join("\n[thincoder-suite] ") : ""
  const effectiveEffNote = effNote + timeoutNote
  // R6 code review 跟进（对齐 advisor.mjs R4 收尾 #4 先例 + eng.mjs 同款点）：后台派发返回并入
  // 配置告警（effectiveEffNote = effort/global/timeout 值回落警告——此前只附着在 job 完成文本，
  // 句柄先返回、告警被推迟到 job 完成才可见）。前缀形态：去头部空行 + 空行分隔；无告警时行为不变。
  const warnPrefix = effectiveEffNote ? effectiveEffNote.replace(/^\n+/, "") + "\n\n" : ""

  // —— 预算 > budgetCap 或显式 background=true（R5 §7.2）且 ctx.jobs 可用 → 后台派发
  //（advisor 先例整体复制；background=true = 长续轮等场景的强制后台通道，无视预算）——
  if (baseTimeout > budgetCap || background) {
    ...
    if (jobs && typeof jobs.start === "function") {
      ...
      try {
            ...
            const done = runCodexTask(deps, {
              ...
              return jobOutcome(handle, {
                ...
                output: "escalate (" + tag + ") ended: codex-cli " + env.code + " — "
                  + (env.userMessage || env.diagnostics || "unknown") + partial + codexFailureAdvisory(env) + effectiveEffNote,
              }, jobPersistOpts)
            }, (e) => {
              clearInFlightJob(threadKey, "escalate")
              codexThreads.delete(threadKey)
        ...
        // R6 code review 跟进：配置告警（warnPrefix）并入派发返回——后台派发即时可见。
        setInFlightJob(threadKey, "escalate", started)
        return warnPrefix + codexJobsDispatchReply({ prefix: "escalate", jobId: started, budgetMs: baseTimeout, budgetCapMs: budgetCap, detail: tag + (resumeThreadId ? " (resume)" : "") })
      } catch (e) {
        console.warn("[thincoder-suite] ctx.jobs 派发失败（" + (e?.message ?? String(e)) + "）——escalate 回落同步执行"
          + (baseTimeout > budgetCap ? "（预算按 budgetCap 截断）" : ""))
      }
    } else {
      // jobs 缺失降级（响亮告警——绝不无声）；background=true 且预算 ≤cap 时不截断（预算本就合规）
      console.warn("[thincoder-suite] ctx.jobs 不可用——escalate codex "
        + (background && baseTimeout <= budgetCap
  ...
  try {
    const env = await runCodexTask(deps, {
      ...
      if (env.threadId) codexThreads.set(threadKey, { threadId: env.threadId, runner, label: tag })
      return "escalate (" + tag + resumeTag + ") post-op report:\n" + (env.text || "(empty report)") + capNote + effectiveEffNote
    }
    codexThreads.delete(threadKey)
    const partial = env.text ? "\nPartial output:\n" + env.text.slice(0, 2000) : ""
    return "escalate (" + tag + ") ended: codex-cli " + env.code + " — " + (env.userMessage || env.diagnostics || "unknown")
      + partial + capNote + codexFailureAdvisory(env) + effectiveEffNote
  } catch (e) {
    codexThreads.delete(threadKey)
    if (signal?.aborted) {
      // 批 6 FR-AP5：前缀 "escalate (tag) aborted." 逐字保留，仅追加溯源后缀（layer settle = 宿主面信号）
      return deathLine(annotateAbort(new Error("escalate (" + tag + ") aborted."), signal, "settle", "caller signal abort"), signal) + capNote
    }
 ...
 * @param task — 任务书（goal/constraints/acceptance）
 * @param model — 候选 'provider:model'（缺省 pool[0]）
 * @param followup — codex 行续轮（延续本会话已保存的 codex thread）
 * @param background — 批 21（FR-1②）三态：省略/true ⇒ dsh 行派后台 job（kind
 *        escalate-dsh，免平台墙钟，dshBackgroundTimeoutMs 兜底防挂死）——省略即后台
 *        （默认翻转）；显式 false ⇒ 强制同步快路径（DP-1：budgetCap 内部截止）。codex 行/
 *        followup 仅显式 true 强制后台派发（无视预算——该判定零改动）。
 * @returns 术后病历文本（background 派发时 = job 句柄 + UI-2 接续指令）
 */
export async function runEscalate(deps, task, model, followup, background) {
  ...
  // 会诊回复里显示的标签与 escalate 候选列表对不上，用户点名会落空）。
  // 批 15（FR-2）：原文点名的是 consult 的**读取面**工具；该工具已随投递改造退役（digest 自动投递），
  // 故此处改指仍在位的会诊入口，语义（标签必须与候选列表同一命名）零改动。
  ...
  if (followup) {
    ...
    return runEscalateCodex(deps, {
      ...
      effNote: "",
      background: background === true, // R5 §7.2：followup 同参数语义（续轮任务往往更长）
    })
  }
  ...
  if (nr.ok && nr.runner.kind === "codex-cli") {
    ...
    const globalNote = (globalRes.warnings ?? []).length > 0
      ? "\n\n[thincoder-suite] " + globalRes.warnings.join("\n[thincoder-suite] ") : ""
    const effRes = await resolveCodexRowEffort(deps, runner, runner.effort, globalRes.globals)
    const effNote = globalNote + (effRes.note ? "\n\n[thincoder-suite] " + effRes.note : "")
    ...
    return runEscalateCodex(deps, {
      ...
      resumeThreadId: null,
      effNote,
      background: background === true, // R5 §7.2：codex 行同样接受 background（强制后台）
    })
  }
  ...
  // 子代理请求（signal 因执行形态而异——同步路径 = budgetCap 内部截止 + 父级 abort 汇入；
  // background=true = job 的 ctrl（cancel → abort / 兜底 deadline abort），spawn 时注入）
  const request = {
    ...
    toolFilter: { deny: ["escalate", "consult_start", "consult_stop", "eng", "eng_coder"] },
  }

  // —— R5 §7.2（D-27）：background=true → dsh 子代理后台 job 派发（免平台墙钟）——
  // run() 闭包内 ctx.subagents.start + await run.result（ctx 引用在 job 生命周期内有效——
  // session 优雅销毁 → job cancel → abort 传递）；dshBackgroundTimeoutMs 兜底 deadline 到点
  // abort（挂起子代理不永悬 job）+ 超时信封（partial 保留）；簿记（touchedFiles 合并 + F12
  // 落盘——后台完成晚于工具返回，必须自查落盘）在 done 成功分支恰好一次（D-20）；失败/中止
  // 不簿记；cancel 传播：job kill → hooks.cancel → ctrl.abort → 子代理 abort。派发抛错 /
  // ctx.subagents 缺失 → 同步 + budgetCap 内部截止 + 响亮告警降级（现状路径）。
  const budgetCap = resolveCodexBudgetCapMs(config)
  // 批 26 搭车①（A26-5）：显式 background=false 指定同步快路径，且预算已达平台 run_code 墙钟
  // ⇒ 预算注定跑不完，文案点明「本路会白等」（US-5：同步派发的墙钟陷阱在派发时说清；
  // 回落形态〔background 省略但 jobs 缺失〕不在此列——eng.mjs 同名前置告警已覆盖它）。
  if (background === false && budgetCap >= PLATFORM_RUN_CODE_WALL_MS) {
    console.warn("[thincoder-suite] escalate background=false 指定同步快路径，且 codexCli.budgetCapMs=" + budgetCap
      + "ms 已达平台 run_code 墙钟 " + PLATFORM_RUN_CODE_WALL_MS + "ms——本路会白等：超过墙钟的预算部分注定被截断"
      + "（如需全预算请走后台 job 或 codex runner）")
  }
  // 批 21（FR-3 / D-32-6）：回落告警双通道——console.warn（既有字面，逐字节不变）+ 工具
  // 返回文本（本批新增；此前只 console ⇒ 翻转后默认走后台，父代理会以为派了后台、傻等一个
  // 不会来的通知 = 静默失败）。与 eng.mjs 同款**尾部后缀**并入下方同步路径的全部返回点
  //（回落分支不 return，落同步路径续行；advisor「回落 note 是后缀」先例同形——不破坏
  // 返回文本首段）。行为仍是回落同步、文案逐字节不变（D-32-5 硬约束）。
  let dshFallbackNote = ""
  const fallbackSuffix = () => (dshFallbackNote ? "\n\n" + dshFallbackNote : "")
  // 批 21（FR-1② / D21-1）：三态判定——省略 ⇒ 派后台 job（新默认，与 eng_coder dsh 路径
  // 同款）；显式 false ⇒ 强制同步快路径（DP-1 方案 A 保留）；非布尔且非 false ⇒ 仍后台
  // （fail-safe，N-9/AC-15）。codex 行判定（runEscalateCodex 内 === true）不动——零漂移（AC-6）。
  if (background !== false) {
    const jobs = getJobsService(deps)
    // Preflight only checks jobs. The subagent service is checked inside run(); once a
    // handle is returned, an unavailable service is a terminal diagnostic failure.
    if (jobs && typeof jobs.start === "function") {
      // D-06（§4.3）：single-flight 派发前二次检查（入口检查之后到 jobs.start 之间的 await
      // 窗口——effort 解析等——内仍可能被他方派发占位；escalate 槽位与同步入口共用）
      const inFlight = checkInFlightJob(threadKey, "escalate")
      if (inFlight) return inFlight
      const ctrl = new AbortController()
      let settled = false
      ...
      const settle = () => {
        settled = true
        if (slotReady) clearInFlightJob(threadKey, "escalate")
      }
      try {
        // R5 §7.2 挂死兜底：dshBackgroundTimeoutMs 到点 abort（挂起的子代理不永悬 job）——
        // 超时信封（partial 保留）附在 outcome；取消经 hooks.cancel → ctrl.abort
        const backstopMs = resolveDshBackgroundTimeoutMs(config)
        const started = jobs.start({
          kind: "escalate-dsh",
          label: "escalate dsh (" + tag + ", background)",
          outputLimitBytes: 131072,
          owner: ownerIdOf(agent),
          run: (handle) => {
            // 批 27 / US-5（D27-4 · A27-8）：落盘 home 探测透传会话 cwd——可选，缺省行为逐字不变
            ...
            if (Number.isFinite(backstopMs) && backstopMs > 0) {
              backstopTimer = setTimeout(() => {
                backstopFired = true
                console.warn("[thincoder-suite] escalate dsh 后台子代理超兜底截止（dshBackgroundTimeoutMs="
                  + backstopMs + "）——abort 子代理（job 永悬防护，R5 §7.2）")
                try { ctrl.abort(timeoutError("escalate dsh background backstop deadline reached", "agent", "dshBackgroundTimeoutMs")) } catch { /* already aborted */ }
              }, backstopMs)
              backstopTimer.unref?.()
            }
            const timeoutEnvelope = (outputText) => {
              const partial = outputText ? "\nPartial output:\n" + outputText.slice(0, 2000) : ""
              return jobOutcome(handle, {
                status: "failed",
                detail: "dshBackgroundTimeoutMs backstop",
                output: "escalate (" + tag + ") ended: dsh 后台子代理超兜底截止（dshBackgroundTimeoutMs="
                  + backstopMs + "）到点，子代理已 abort——job 永悬防护（R5 §7.2，partial 保留）。"
                  + partial + codexFailureAdvisory({ text: outputText, code: "ABORTED" }) + effNote,
              }, jobPersistOpts)
            }
            const done = (async () => {
              let sub
              try {
                if (!ctx?.subagents || typeof ctx.subagents.start !== "function") {
                  return jobOutcome(handle, { status: "failed", detail: "ctx.subagents unavailable", output: "escalate (" + tag + ") failed: ctx.subagents is unavailable inside background job (cannot recover after dispatch)" }, jobPersistOpts)
                }
                sub = await ctx.subagents.start("spawn", { ...request, signal: ctrl.signal })
                const result = await sub.result
                const outputText = (result?.output ?? [])
                  .filter(b => b?.type === "text").map(b => b.text ?? "").join("\n").trim()
                // 竞态分支：兜底 deadline abort 与 run.result 正常返回的竞态（子代理可能以
                // stopReason=aborted 正常返回而非 reject）——backstopFired 后不再按成功处理
                if (backstopFired) return timeoutEnvelope(outputText)
                const stopReason = result?.stopReason ?? "unknown"
                if (stopReason !== "completed") {
                  // D-20：失败/中止不簿记——partial 内 Touched 行仅 advisory（D-11）
                  const diag = result?.diagnostic ? " — " + result.diagnostic : ""
                  const partial = outputText ? "\nPartial output:\n" + outputText.slice(0, 2000) : ""
                  return jobOutcome(handle, {
                    status: "failed",
                    detail: stopReason,
                    output: "escalate (" + tag + ") ended: " + stopReason + diag
                      + partial + codexFailureAdvisory({ text: outputText, code: null }) + effNote,
                  }, jobPersistOpts)
                }
                // D-20：簿记在 done 成功分支恰好一次（codex jobs done 同语义——后台完成晚于
                // 工具返回，advisor 的 F12 写点不会再跑，必须自查落盘；fail-safe 仅 warn）
                ...
                return jobOutcome(handle, {
                  status: "completed",
                  detail: "escalate delivered",
                  output: "escalate (" + tag + ") post-op report:\n" + (outputText || "(empty report)") + effNote,
                }, jobPersistOpts)
              } catch (e) {
                // 兜底 deadline / cancel 的 reject 兜底；start 抛错（不可达等）同落此
                if (backstopFired) return timeoutEnvelope("")
                if (ctrl.signal.aborted || e?.name === "AbortError") {
                  ...
                  return jobOutcome(handle, {
                    ...
                    output: deathLine(annotateAbort(new Error("escalate (" + tag + ") aborted."), src, layer, "dsh subagent abort"), src),
                  }, jobPersistOpts)
                }
                return jobOutcome(handle, { status: "failed", detail: String(e?.message ?? e), output: "escalate (" + tag + ") failed to start: " + (e?.message ?? String(e)) }, jobPersistOpts)
              } finally {
                if (backstopTimer) clearTimeout(backstopTimer)
                settle()
                try { if (sub) await sub.dispose() } catch { /* already disposed */ }
              }
            })()
            done.catch(() => { /* 观察者兜底：reject 已转为 outcome */ })
            return { cancel: (reason) => { try { ctrl.abort(reason) } catch { /* noop */ } }, done }
        ...
        // D-06：登记槽位（settle 时经 done finally 清除）；D-05：branded string 直接渲染；
        // UI-2（D-09）：句柄 + 接续指令——完成通知只含一行指针，全文经 job_output 读取
        setInFlightJob(threadKey, "escalate", started)
        slotReady = true
        if (settled) clearInFlightJob(threadKey, "escalate")
        return jobsDispatchReply({
          prefix: "escalate",
          jobId: started,
          budgetMs: backstopMs, // dsh 后台任务的到期视界 = 挂死兜底截止（免墙钟；同步钳制不适用）
          detail: tag + " · dsh 子代理后台执行（挂死兜底 dshBackgroundTimeoutMs）",
        })
      } catch (e) {
        // R5 §7.2 派发降级：jobs.start 抛错 → 同步 + 内部截止 + 响亮告警（不静默吞）
        dshFallbackNote = "[thincoder-suite] ctx.jobs 派发失败（" + (e?.message ?? String(e))
          + "）——escalate background=true 回落同步执行（budgetCap 内部截止）"
        console.warn(dshFallbackNote)
      }
    } else {
      // jobs / ctx.subagents 缺失 → 同步降级（budgetCap 内部截止，响亮告警——绝不无声）
      dshFallbackNote = "[thincoder-suite] escalate background=true 不可用（"
        + (jobs && typeof jobs.start === "function" ? "ctx.subagents 缺失" : "ctx.jobs 缺失")
        + "）——回落同步执行（budgetCap 内部截止）"
      console.warn(dshFallbackNote)
    }
  }

  // DP-1 方案 A（R2 §4.2）：dsh 子代理路径内部截止——await run.result 此前无界等待（600s
  // 墙钟下超限会被静默截断且无术后报告）。到点 abort + 响亮告警。批 21（FR-1②）后本同步
  // 快路径只在两种形态到达：显式 background=false（短任务逃生口）与 jobs 缺失/派发失败
  // 回落（FR-3 告警已随返回文本可见）；长任务默认已后台，回落态请装 dsh-jobs-local +
  // dsh-tool-jobs（或改走 codex runner / 拆分 stages）。
  ...
  const dshTimer = setTimeout(() => {
    ...
    try { dshCtrl.abort(timeoutError("escalate dsh subagent deadline reached", "agent", "budgetCapMs")) } catch { /* already aborted */ }
  }, budgetCap)
  dshTimer.unref?.()
  ...
  let run
  try {
    run = await ctx.subagents.start("spawn", { ...request, signal: dshCtrl.signal }) // 截止 + 父级 abort 二合一（父 signal 经 forwardSignal 汇入）
  } catch (e) {
    clearTimeout(dshTimer)
    return "escalate (" + tag + ") failed to start: " + (e?.message ?? String(e)) + fallbackSuffix()
  }
  ...
  try {
    ...
    if (signal?.aborted || e?.name === "AbortError") {
      ...
      return deathLine(annotateAbort(new Error("escalate (" + tag + ") aborted."), src, layer, "dsh subagent abort"), src) + fallbackSuffix()
    }
    return "escalate (" + tag + ") error: " + (e?.message ?? String(e)) + fallbackSuffix()
  } finally {
    clearTimeout(dshTimer)
  ...
  if (dshTimedOut) {
    // R3 code review 收尾 ②（R4 折入，设计 §6.5）：竞态分支（截止 abort 与 run.result 正常
    // 返回的竞态——上方 throw 分支未触发）补 codexFailureAdvisory，与 eng 同形态：
    // partial 内 Touched 行 advisory 解析（提示可见但不并入审计范围，D-11/D-20）。
    return "escalate (" + tag + ") ended: dsh 子代理路径超内部截止（budgetCapMs=" + budgetCap
      + "）到点，子代理已 abort——dsh 子代理路径受平台墙钟约束，超限任务被终止；如需长任务请走 codex runner 或拆分 stages。"
      + "\nPartial output:\n" + outputText.slice(0, 2000) + codexFailureAdvisory({ text: outputText, code: null }) + effNote + fallbackSuffix()
  }
  ...
  mergeEscalateTouched(state, parseTouchedFiles(outputText))
  return "escalate (" + tag + ") post-op report:\n" + (outputText || "(empty report)") + effNote + fallbackSuffix()
}
