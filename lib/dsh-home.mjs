 ...
 * 均未命中时探测 DSH 标准 home ~/.dsh（2026-09-07 本地实证：desktop 构建插件进程无 DSH_HOME
 * 且 cwd 与真实 home 无祖先关系）。无可用 home → null（调用方各自 fail-safe：save 仅 warn）。
...
export function pickDshHome(dshHomeOverride, cwdHint) {
  ...
  const viaProbe = probeProfileRoot(typeof cwdHint === "string" && cwdHint.trim() !== "" ? cwdHint.trim() : process.cwd())
  if (viaProbe) return viaProbe
  // 本地部署实证（2026-09-07）：desktop 构建下插件进程无 DSH_HOME 环境变量，且其 cwd
  // （应用目录，与真实 home 无祖先关系）向上探测不到 profile 根 → 设置页保存 user 配置失败
  // （"config store path not resolvable"）。兜底：探测 DSH 标准 home ~/.dsh（含 sessions/ 与
  // settings.yaml 即视为 profile 根，与 token-store 特征一致；portable/有 env 的部署不受影响，
  // 仍走 override/env/cwd 优先路径）。
