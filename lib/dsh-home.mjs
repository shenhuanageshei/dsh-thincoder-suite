...
export function pickDshHome(dshHomeOverride, cwdHint) {
  ...
  // 兜底：探测 DSH 标准 home ~/.dsh（含 sessions/ 与 settings.yaml 即视为 profile 根，
  // 与 token-store 特征一致；portable/有 env 的部署不受影响，仍走 override/env/cwd 优先路径）。
 ...
 * @param {{delays?: number[], maxAttempts?: number, write?: () => void, onEnoent?: (e: Error) => boolean}} [opts]
 ...
 * @param {{delays?: number[], maxAttempts?: number}} [opts] 注入缝（§5.1：测试传 delays [1,1,1]）
 */
