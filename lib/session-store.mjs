...
// advisorOverride, lastSeen, advisorGeneration（R2 §4.4/D-10 代际——条件字段，经
// advisor.mjs sessionStateViewWithGeneration 写点拼入）, lastDesignDocKey（批 4 §12 FR-G9——设计
// 评审链的**文档集作用域键**，字符串可选，同经 sessionStateViewWithGeneration 写点拼入）}
// ——**绝不含 designToken/pendingDesignToken**
// （N4：F10 token-store 单路管理凭证，双盘写 = 双事实源，撤销要走双写同步，任何一路失败即「撤销失效/复活」），
// 亦**绝不含 D-30 的 pendingDocHash/pendingDocPaths**（FR-T5 文档集指纹：与 token 同域的第二存储语义——
// 指纹随 token 记录落 design-tokens.json；本文件的白名单 normalizeRestored 天然将其丢弃，AC-20 锁死。
// 此处显式登记，防未来把它加进白名单而制造第二个事实源）。
...
// 原子写 = dsh-home.writeFileAtomic（tmp+rename + rename 白名单错误有界重试、耗尽向上抛——批 20；
// 评审 #4：多会话单文件直写 = 崩半写丢全部会话状态）。
 ...
 * effort/timeoutMs/runner）+ includeProjectGuide。值级校验（枚举/数值区间）留在 advisor 解析链——
 * §2.1「白名单校验链已存在」（resolveAdvisorRoute 对 override 字段逐个校验、非法忽略并 N4 警告），
 * 此处不复制值校验逻辑（单一事实源）；结构非法的键直接丢弃。
 * R4 §6.2（D-13 三面同步）：组字段白名单增 runner——会话覆盖的 runner 此前落盘即丢
 * （重启后 resolve 链看不到 codex runner 覆盖），与 advisor_config 工具面/apply-session 面对齐。
 */
function sanitizeAdvisorOverride(v) {
  ...
  for (const gk of ["round1", "convergence"]) {
    ...
    const gOut = {}
    for (const f of ["provider", "model", "effort", "timeoutMs", "runner"]) {
...
export function normalizeRestored(entry) {
  ...
  // R2 §4.4（D-10）：advisorGeneration 持久化白名单（整数 ≥ 0）。条件字段：写点统一经
  // advisor.mjs sessionStateViewWithGeneration 拼入；不带该字段的视图（viewOfSessionState
  // 形态/旧条目）不引入键——键集兼容既有白名单断言。非法值不保留（恢复侧按 0 兜底）。
  if (Number.isInteger(entry.advisorGeneration) && entry.advisorGeneration >= 0) {
    out.advisorGeneration = entry.advisorGeneration
  }
  // 批 4 §12（FR-G9/N-7）：lastDesignDocKey 持久化白名单（非空字符串；**可选条件字段**——同
  // advisorGeneration 模式，写点经 advisor.mjs sessionStateViewWithGeneration 拼入）。链作用域
  // 键必须落盘（advisorRound 是落盘的；键不落盘会在重启后失配 = 同一文档集被误判为新链）。
  // 值 = advisor.mjs `designChainKey(documents)`（= sha256Hex(designDocKey(...))）的 hex——
  // **不是**裸路径表：既有 AC-20 禁止文档路径出现在本镜像里（见 designChainKey 的 JSDoc）。
  // 空串视为无值不引入键。未确认面（§9 G-5）：落盘继承 D-30 已登记的多进程 last-writer-wins 风险面。
  if (typeof entry.lastDesignDocKey === "string" && entry.lastDesignDocKey !== "") {
    out.lastDesignDocKey = entry.lastDesignDocKey
  }
  return out
}
