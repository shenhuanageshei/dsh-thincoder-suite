...
// ————————————— D-25（R4 收尾微修复轮）：TOKEN_SECRET 密钥源 —————————————
// 旧缺陷：回落源码内公开默认值（"thincoder-default-secret"）——默认部署下 design token
// （eng_coder 写门禁凭证）可被任何知悉默认值者伪造。新解析链（优先级）：
// ① env THINCODER_TOKEN_SECRET（显式运维密钥）；
// ② $DSH_HOME/.thincoder/token-secret 持久化密钥（复用 dsh-home.mjs 既有路径解析，与
//    token-store 同链；缺失 → 生成 crypto.randomBytes(32).hex 并原子写持久化 + chmod 0600
//    尽力而为——重启复用同一密钥，在途 token 不因重启失效）；
// ③ 无 env 且路径不可解析（无持久化面）→ 回落公开默认值 + 响亮告警「门禁不可信」。
// 密钥变更会使既有在途 token 失效（HMAC 签名不匹配 → 重跑 design review 重铸，TTL 1h
// 限制失效面）——首次升级启动（公开默认 → 随机持久化）即发生一次，属 D-25 修复的预期
// 代价（无旧签名兼容通道：保留旧默认验签 = 保留伪造通道）。进程内一经解析即稳定（模块
// 级单例 tokenSecret，首次 token 操作时解析——不在模块顶层急解析：宿主/测试进程对 env 的
// 调整可能发生在 import 之后，顶层急解析会读到导入时刻的陈旧 env）。持久化写失败 → 仍用
// 已生成密钥（进程内稳定）+ warn：重启将重新生成、在途 token 届时失效（诚实可见）。
const TOKEN_SECRET_FALLBACK = "thincoder-default-secret"
const TOKEN_SECRET_SUBDIR = ".thincoder"
const TOKEN_SECRET_FILE = "token-secret"

/** D-25：密钥文件路径（$DSH_HOME/.thincoder/token-secret——dsh-home.mjs 既有解析链，与 token-store 同源）。 */
export function resolveTokenSecretPath(dshHomeOverride, cwdHint) {
  const home = pickDshHome(dshHomeOverride, cwdHint)
  return home ? join(home, TOKEN_SECRET_SUBDIR, TOKEN_SECRET_FILE) : null
}

/**
 * D-25：TOKEN_SECRET 无状态解析（导出供测试直接驱动三形态；生产经 tokenSecret() 单例消费）。
 * 优先级：env THINCODER_TOKEN_SECRET → 持久化文件（缺失则 randomBytes(32).hex 生成 + 原子写
 * + chmod 0600 尽力而为）→ 公开默认值 + 响亮告警。env 值原样使用（空白串视为未设）。
 * @param {object} [env] — 环境（测试注入缝；缺省 process.env）
 * @param {string} [dshHomeOverride] — DSH_HOME 目录注入缝（对齐 token-store storPathOverride 先例）
 * @param {string} [cwdHint] — 探测起点（对齐 pickDshHome cwdHint 语义）
 * @returns {string} HMAC 签名密钥
 */
export function resolveTokenSecret(env, dshHomeOverride, cwdHint) {
  const e = (env && typeof env === "object") ? env : {}
  const fromEnv = typeof e.THINCODER_TOKEN_SECRET === "string" && e.THINCODER_TOKEN_SECRET.trim() !== ""
    ? e.THINCODER_TOKEN_SECRET : null
  if (fromEnv !== null) return fromEnv
  const filePath = resolveTokenSecretPath(dshHomeOverride, cwdHint)
  if (filePath) {
    try {
      const persisted = readFileSync(filePath, "utf8").trim()
      if (persisted !== "") return persisted
    } catch { /* 缺失/不可读 → 生成新密钥（空/损坏文件就此自愈覆盖） */ }
    const secret = randomBytes(32).toString("hex")
    try {
      writeFileAtomic(filePath, secret + "\n")
      try { chmodSync(filePath, 0o600) } catch { /* 0600 尽力而为（Windows 上近似 no-op） */ }
    } catch (err) {
      console.warn("[thincoder-suite] token-secret 持久化失败（" + (err?.message ?? String(err))
        + "）——本轮使用进程内随机密钥；重启将重新生成，在途 design token 届时失效")
    }
    return secret
  }
  console.warn("[thincoder-suite] design token 门禁使用公开默认密钥，不可信——设置 THINCODER_TOKEN_SECRET 或修复 DSH_HOME")
  return TOKEN_SECRET_FALLBACK
}

// 模块级单例（D-25：进程内稳定——首次 token 操作解析一次并缓存）。
let tokenSecretSingleton
/** 测试缝：清模块级密钥缓存（模拟进程重启后的重新解析——下一次 token 操作走完整解析链）。 */
export function resetTokenSecretCacheForTests() { tokenSecretSingleton = undefined }
function tokenSecret() {
  if (tokenSecretSingleton === undefined) tokenSecretSingleton = resolveTokenSecret(process.env)
  return tokenSecretSingleton
}
...
// 导出供测试直接驱动（对齐 resolveTokenSecret 等既有导出缝先例）：D-29 需要断言
// 「user 层生效配置真的决定 token 有效期」，只有驱动真实铸造路径才能锁住消费点形态。
export function generateDesignToken(config) {
  ...
  const signature = createHmac("sha256", tokenSecret()).update(payload).digest("hex").slice(0, 16)
...
export function validateDesignToken(token) {
  ...
  const payload = uuid + ":" + expiresAt
  const expectedSig = createHmac("sha256", tokenSecret()).update(payload).digest("hex").slice(0, 16)
  return signature === expectedSig
}

/**
 * 批准码派生（R2）：code = HMAC-SHA256(当前密钥, designToken).hex.slice(0, 8)。
...
export function designApprovalCode(designToken) {
  return createHmac("sha256", tokenSecret()).update(String(designToken ?? "")).digest("hex").slice(0, 8)
}
            ...
            run: () => {
              ...
              if (Number.isFinite(backstopMs) && backstopMs > 0) {
                backstopTimer = setTimeout(() => {
                  ...
                  try { ctrl.abort(new Error("dsh background backstop deadline reached")) } catch { /* already aborted */ }
                }, backstopMs)
                backstopTimer.unref?.()
              }
              const done = (async () => {
                try {
                  ...
                  return {
                    ...
                    output: finalize(loop) + dshEffNote + backstopNote,
                  }
                } catch (e) {
                  // 兜底 deadline / cancel 的 reject 兜底（runAdvisorToolLoop 内部已把常规失败转
                  // 字符串——此处只接异常形态：不可达/解析失败等）
                  const msg = String(e && e.message ? e.message : e)
                  return { status: "failed", detail: msg, output: "Advisor: review failed — " + msg + dshEffNote }
                } finally {
