              ...
              run: (handle) => {
                const done = runTask(route.timeoutMs, ctrl.signal).then((env) => {
                  ...
                  if (advisorGenerationOf(state) !== genAtDispatch) {
                    ...
                    }, { pathForm: "session-state" })
                  }
                  // finalize 在 done settle 前执行：轮次推进/prior/token 恰好一次
                  return jobOutcome(handle, { status: env.ok ? "completed" : "failed", detail: env.ok ? "review delivered" : ("codex-cli " + env.code), output: finalize(result, codexSettlementHint(env)) + effNote }, { pathForm: "session-state" })
                }, (e) => {
                  clearInFlightJob(sid, "advisor")
