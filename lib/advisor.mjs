...
// design token 协议 v2：token 每评审会话只铸造
...
import { sessionState, viewOfSessionState } from "./state.mjs"
...
// 批 7（D-P9）：评审文档集校验的判据 = path-kind.mjs 的单一权威（本地副本 isDocFile 已删除）
...
/**
 * 工具结果消息（D-41：0.1.7 消息模型变更）。
 *
 * DSH 0.1.6 的 `createToolResultMessage` 逐字产出的就是**旧形状**（role:'user' +
 * content:[{type:'tool-result', toolCallId, content, isError}] + source:{kind:'tool',callId}），
 * 所以旧写法当年是正典、不是错。0.1.7 改成**独立 tool 角色消息**：
 *   { role:'tool', toolCallId, content(原始结果块数组), isError, source:{kind:'tool',callId} }
 * 并**删除了旧形状的兼容**：dsh-llm-pi-ai 的 userContent 去掉了 case "tool-result"（静默丢弃 ⇒
 * 留下空 user 消息 + 无人应答的 tool_call ⇒ 上游 400 {"code":"1213"}），dsh-llm-deepseek 的
 * user 分支遇到该块直接 unsupported() 抛 UNSUPPORTED_CONTENT。
 *
 * 触发时机：第一轮请求正常，模型一旦调用 read/grep，**第二轮**带上工具结果即炸——两条路由两种
 * 报错，根因同一处。契约回归锁见 test/dsh017-compat.test.mjs。
 */
...
/** D-41：工具结果消息判据（唯一实现来源——压缩守卫与摘要共用；0.1.7 起是独立 tool 角色消息）。 */
const isToolResultMsg = (m) => m?.role === "tool"

/**
 * 压缩：保首条 user + 最近 20 条，中间以本地摘要替换（对齐 thincoder run.mjs）。
 * 边界守卫（D-41 改判）：0.1.7 下工具结果是**独立 tool 消息**，协议要求它紧跟在带 tool-call 的
 * assistant 之后。窗口起点若落在 tool 消息上，其配对前驱会被切走 ⇒ 必回退；回退停下的第一个非
 * tool 消息就是该 tool 的配对 assistant（消息顺序保证），配对因此完整。
 * 退化边界（新增）：一路回退到 1 仍落在 tool 上 ⇒ 这些 tool 的配对前驱本来就不在数组里
 * （畸形/伪造输入）⇒ **向前跳过并丢弃**，绝不产出以孤儿 tool 开头的窗口——deepseek 侧对孤儿会抛
 * "tool result has no matching call"，那会把「每次必炸」变成「压到 20 条以后才炸」，更难查。
...
export function compactMessages(messages) {
  ...
  while (start > 1 && isToolResultMsg(messages[start])) start--
  ...
  const toolCount = old.filter(isToolResultMsg).length
  const keyFiles = [...new Set(
    old
      .filter(isToolResultMsg)
      .map(m => (m.content?.[0]?.text ?? "").split("\n")[0]?.slice(0, 50))
      .filter(Boolean),
  )].slice(0, 15)
...
// 批 7（D-P9）：本地副本 isDocFile 已删除——文档判据单一权威 = path-kind.mjs 的 isDocPath
// （import 见文件头）。删除理由：该子问题被实现了两遍且各有盲区，矛盾全部出在重复上。
 ...
 * 批 26（D-45）：job_output 的读取面实测会崩（平台自产作业同错，读取面不在本仓）⇒ 文案改为
 * **首选读落盘文件**（$DSH_HOME/.thincoder/jobs/<jobId>.txt——jobOutcome 收口的自持落盘），
 * job_output 作为附加读取手段。四条机制（advisor/consult/eng/escalate）的派发回复都经
 * jobsDispatchReply 汇到本常量——一处改全改（US-2：别的会话不必再问「怎么绕」）。 */
export const JOBS_CONTINUATION_INSTRUCTION =
  "等待完成通知后再继续；长任务勿用 job_output wait 阻塞等待（平台等待上限 600s）。报告全文首选读落盘文件 $DSH_HOME/.thincoder/jobs/<jobId>.txt（<jobId> = 本条派发返回的 job id），job_output 作为附加读取手段（完成通知只含一行指针）。"
...
/**
 * 平台 run_code 单次调用墙钟（批 26 搭车①：升为**共享常量**——eng/escalate 的同步快路径
 * 「预算 vs 墙钟」判断与本文件 D-16 提醒同一事实源；eng.mjs 原模块内副本 PLATFORM_WALL_CLOCK_MS
 * 由本常量取代，值不变 600000）。插件读不到平台的 maxWallMs（无注入面，D-16）⇒ 只用于前置告警。
 */
export const PLATFORM_RUN_CODE_WALL_MS = 600_000
...
export function checkInFlightJob(sessionId, mechanism) {
  ...
  return "Error: 该会话已有在飞的 " + existing.mechanism + " 后台任务（job " + existing.jobId
    + "）——等待完成通知后再继续（完成通知只含一行指针，全文见盘上文件 $DSH_HOME/.thincoder/jobs/<jobId>.txt（首选；job_output 为附加）；长任务勿用 "
 ...
 * @param opts  { agent, config, signal, reviewType, documents, paths, storPathOverride? }
 *               — storPathOverride = F10 存储路径注入缝（测试用临时目录，
 *               对齐 stallMsOverride 先例，非用户配置；缺省走 token-store 的 DSH_HOME 解析）。
 *               **注（批 7 分歧审计 #4）**：调用方 `lib/index.mjs` 仍传 `configDefaultEngineering`，
 *               但本模块**已无该参数的消费点**（工程模式判定的消费点在 `eng.mjs` / `escalate.mjs`；
 *               本函数入口只解构 `agent` 与 `signal`）。该键**不参与** advisor 的任何分支，
 *               保留仅为调用方签名兼容——不得据它推断「本次评审跑在工程模式」。
...
export async function runAdvisorReview(deps, opts) {
  ...
  // ————————————— 批 21（FR-2 / D21-2 / D-32-1）：跨机制护栏（code 型评审 × eng/escalate 在飞） —————————————
  // 默认后台（FR-1）后，「eng 交付 ⇒ 簿记已发生 ⇒ code 评审看到 fresh 状态」这条由父代理
  // 阻塞天然保证的因果链断掉——touchedFiles 合并 / advisorRound 重置 / prior 清空 / 代际
  // bump 都在 job settle 回调里发生 ⇒ 完成通知前发起的 code 评审：scope 兜底看不到新改动、
  // prior/轮次是上一交付链残留、交错时代际校验让那轮评审白跑（D-10 保住状态、保不住那轮
  // 的钱）。护栏 = 复用 checkInFlightJob（同复合键 sessionId+mechanism、同 settle 清理点
  // ——**只读检查、不改槽位机制本体**）：eng/escalate 任一槽位在飞 ⇒ 拒绝（文本点名 job id
  // 并指引「等完成通知后再继续」）。design 型评审不受影响（与 eng 交付链无此耦合）。
  // 零副作用预检——位置先于 F11 类型切换/链重置（同上方三振预检纪律：被拒不得重置在途
  // 评审序列）；重启后槽位失忆是已登记边界（R-66），不在本批修。
  if (reviewType === "code") {
    for (const mechanism of ["eng", "escalate"]) {
      const crossInFlight = checkInFlightJob(sid, mechanism)
      if (crossInFlight) return crossInFlight
    }
  }
  ...
  if (reviewType === "design") {
    ...
    // D-44：显式传**会话 cwd** 作基座——documents 里的相对路径此前按 resolve 的默认基座
    // （宿主进程 cwd = profile 目录，**不是**项目目录）解析 ⇒ 一律读不到 ⇒ fail-closed 不写指纹
    // （续期门控从未生效、Guard E 无可比对象、诊断文案还把它归错因）。叶子模块不猜基座（D25-3）。
    const union = [...(Array.isArray(state.pendingDocPaths) ? state.pendingDocPaths : []), ...(documents ?? [])]
    const dh = computeDocHash(union, cwd)
  ...
  // 批 7（D-P5/J7）· 批 13（R-6）：注入门 = **声明键存在**（不再看模式标志）；三个声明键取自
  // 生效全局配置（都是 global-only 的项目属性，见设计档 §5.1 图 1 的最小面集）：
  // standardsDoc（两面注入的工程标准段）· documentMapDoc（仅 design 的文档地图）·
  // criteriaDoc（仅 code 的判据正文——批 13 新增的独立键）。
  const userText = buildAdvisorUserMessage({
    cwd, history, state, reviewType, designToken, documents, paths, includeProjectGuide,
    standardsDoc: config?.advisor?.standardsDoc,
    documentMapDoc: config?.advisor?.documentMapDoc,
    criteriaDoc: config?.advisor?.criteriaDoc,
  })
              ...
              run: (handle) => {
                ...
                const done = runTask(route.timeoutMs, ctrl.signal).then((env) => {
                  ...
                  if (advisorGenerationOf(state) !== genAtDispatch) {
                    ...
                    }, { pathForm: "session-state", ...jobPersistOpts })
                  }
                  // finalize 在 done settle 前执行：轮次推进/prior/token 恰好一次
                  return jobOutcome(handle, { status: env.ok ? "completed" : "failed", detail: env.ok ? "review delivered" : ("codex-cli " + env.code), output: finalize(result, codexSettlementHint(env)) + effNote }, { pathForm: "session-state", ...jobPersistOpts })
                }, (e) => {
                  clearInFlightJob(sid, "advisor")
