...
import { resolveSupportedEffort } from "./effort-resolve.mjs"
import {
...
const LLM_MAX_TOKENS = 16384            // advisor 单次 LLM 输出预算（2026-09-05 热修 8192→16384：推理型模型 high 档在 8192 内烧光输出预算致 D-01 空响应；R1 改为 advisor.maxOutputTokens 可配置）
  ...
  try {
    ...
    if (codexWanted && failCount >= CODEX_FAILURE_FALLBACK_THRESHOLD) {
      ...
      const fb = await runAdvisorToolLoop(deps, {
        ...
        system, firstUserText: userText, cwd, signal, timeoutMs: fbRoute.timeoutMs,
        sessionId: sid,
      })
      return finalize(fb + (effRes.note ? "\n\n[thincoder-suite] " + effRes.note : "")
        ...
        + fbRoute.provider + ":" + fbRoute.model + "），下一轮将重试 codex。")
    }
    ...
    return finalize(effRes.note ? loop + "\n\n[thincoder-suite] " + effRes.note : loop)
  } catch (e) {
