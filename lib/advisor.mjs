...
const ADVISOR_TIMEOUT_MIN_MS = 1000             // timeoutMs 合法区间（与 advisor_config 工具 coercion 一致，§3.6）
const ADVISOR_TIMEOUT_MAX_MS = 3_600_000
...
function isValidEffort(v) { return typeof v === "string" && EFFORT_LEVELS.includes(v) }
function isValidTimeoutMs(v) {
  return typeof v === "number" && Number.isFinite(v)
    && v >= ADVISOR_TIMEOUT_MIN_MS && v <= ADVISOR_TIMEOUT_MAX_MS
}
/** provider/model 字段只认非空字符串（注册表检测属二期 UI，§3.9）。 */
function isModelField(v) { return typeof v === "string" && v.trim() !== "" }
 ...
 * sessionState.advisorOverride 保持原样（N4）。一期无全局写入（仍编辑 cordis.patch.yml，§3.9）。
...
export function runAdvisorConfigTool(text, deps) {
  ...
  if (action === "get") {
    ...
    const includeProjectGuide = resolveIncludeProjectGuide({ config: deps?.config, override })
    const lines = [
      "advisor_config (session " + (deps?.sessionId ?? "?") + "):",
      "session override: " + (override ? JSON.stringify(override) : "(none)"),
      "global advisor config: " + JSON.stringify(deps?.config?.advisor ?? {}),
      "includeProjectGuide: " + includeProjectGuide.value + " (source: " + includeProjectGuide.source + ")",
      "effective route:",
      describeGroup("round1", deps?.config, override, deps?.agentOpts, 0),
      describeGroup("convergence", deps?.config, override, deps?.agentOpts, 1),
    ]
    return lines.join("\n")
  }
...
export async function runAdvisorReview(deps, opts) {
  ...
  if (state.lastReviewType && state.lastReviewType !== reviewType) {
