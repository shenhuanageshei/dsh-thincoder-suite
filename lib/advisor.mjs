...
import { normalizeRunnerValue, resolveCodexCliGlobals, runCodexTask, CODEX_TASK_PREAMBLE } from "./codex-adapter.mjs"
...
//    下一轮自动回落 dsh 路由一轮（响亮告警，非静默），计数重置后 codex 下轮重试。
...
export function resolveAdvisorRoute({ config, override, agentOpts, advisorRound, ignoreRunner }) {
  ...
  if (runner && runner.kind === "codex-cli") {
    ...
    const codexModel = runner.model
      ?? (typeof cliCfg.model === "string" && cliCfg.model.trim() !== "" ? cliCfg.model.trim() : null)
...
/**
 * D-01 空响应/错误无消息分类行（R1 观测部分——只让文本可归因；重试/烧轮次语义留 R3）。
 * 三形态区分（登记表 D-01：finish null / stop 零块 / error 无 message）+ finish=length
 * 显式分类（生产复现根因：maxTokens 在推理阶段耗尽 → length 零文本块）。
 */
...
export async function runAdvisorToolLoop(deps, opts) {
  ...
  while (true) {
    ...
    if (toolCalls.length === 0) {
      ...
      if (body) return body
      // D-01 观测（R1 部分）：空响应分类行 + 观测字段（三形态区分 + length 显式分类）——
      // 返回语义不变（不重试、不烧轮次——改动留 R3），只让文本可归因。
      return "Advisor: (empty response — review was inconclusive)\n"
...
export async function runAdvisorReview(deps, opts) {
  ...
  try {
    ...
    // 一轮（响亮告警非静默；计数重置后 codex 下轮重试——网络类瞬态故障可自愈）。
    ...
    const capCfg = (config.codexCli && typeof config.codexCli === "object") ? config.codexCli.budgetCapMs : null
    // 未配置 → 默认 540000（低于平台 run_code 默认墙钟 600s，留 envelope 返回余量）
    const budgetCap = Number.isInteger(capCfg) && capCfg > 0 ? capCfg : 540000

    // —— 智能回落：codex 连续 2 次失败 → 本轮自动回落 dsh 路由（响亮告警；计数清零后下轮重试）
    if (codexWanted && failCount >= CODEX_FAILURE_FALLBACK_THRESHOLD) {
      ...
      //（looksLikeReview 启发式不该被后缀推过 200 字符阈值——D-19 prior 纯净化在 R3 收口）
      return finalize(fb + (effRes.note ? "\n\n[thincoder-suite] " + effRes.note : "")
        + "\n\n[thincoder-suite] warning: codex runner 连续 " + failCount + " 次失败——本轮已自动回落 dsh 路由（"
        + fbRoute.provider + ":" + fbRoute.model + "），下一轮将重试 codex。") + fbCapNote
    }
    ...
    if (codexWanted) {
      ...
      if (route.timeoutMs > budgetCap) {
        // ctx 经 deps 注入（index.mjs execute 处）；缺失 → 同步降级
        const jobs = (() => { try { return deps?.ctx?.get?.("jobs") ?? null } catch { return null } })()
              ...
              run: () => {
                const done = runTask(route.timeoutMs, ctrl.signal).then((env) => {
                  ...
                  if (env.ok && env.text) {
                    ...
                    result = "Advisor: review failed (codex-cli " + env.code + ") — " + (env.userMessage || env.diagnostics || "unknown error") + partial + fallbackNote + effNote
                  }
                  // finalize 在 done settle 前执行：轮次推进/prior/token 恰好一次
                  return { status: env.ok ? "completed" : "failed", detail: env.ok ? "review delivered" : ("codex-cli " + env.code), output: finalize(result) }
                }, (e) => ({ status: "failed", detail: String(e && e.message ? e.message : e), output: "Advisor: review failed — " + String(e && e.message ? e.message : e) }))
                done.catch(() => { /* 观察者兜底：reject 已转为 outcome */ })
                return { cancel: (reason) => { try { ctrl.abort(reason) } catch { /* noop */ } }, done }
              },
            })
            return "Advisor: codex review started as background job " + (started && started.id ? started.id : "?")
              + "（模型 " + (route.model ?? "codex 默认") + " · 预算 " + Math.round(route.timeoutMs / 1000) + "s）。"
              + "完成时会话内自动收到通知（含评审全文与轮次推进）；也可用 job_output 主动读取。"
          } catch (e) {
        ...
        if (env.ok && env.text) {
          codexFailureCount.delete(sid)
          result = env.text + "\n\n[thincoder-suite] warning: 预算已按 codexCli.budgetCapMs=" + budgetCap + "ms 截断执行（平台墙钟内）" + effNote
        } else {
          const newCount = Math.min((codexFailureCount.get(sid) ?? 0) + 1, 99)
          codexFailureCount.set(sid, newCount)
          const fallbackNote = newCount >= CODEX_FAILURE_FALLBACK_THRESHOLD
    ...
    // D-17 截断告警在 finalize 之后追加：插件元数据不参与 completed 判定/prior 存储
    //（looksLikeReview 启发式不该被后缀推过 200 字符阈值——D-19 prior 纯净化在 R3 收口）
    return finalize(effRes.note ? loop + "\n\n[thincoder-suite] " + effRes.note : loop) + dshCapNote
  } catch (e) {
