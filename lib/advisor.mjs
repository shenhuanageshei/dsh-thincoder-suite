            ...
            const started = jobs.start({
              ...
              run: (handle) => {
                const done = runTask(route.timeoutMs, ctrl.signal).then((env) => {
                  ...
                  return jobOutcome(handle, { status: env.ok ? "completed" : "failed", detail: env.ok ? "review delivered" : ("codex-cli " + env.code), output: finalize(result, codexSettlementHint(env)) + effNote })
                }, (e) => {
                  clearInFlightJob(sid, "advisor")
