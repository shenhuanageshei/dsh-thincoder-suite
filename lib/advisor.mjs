...
import { sha256Hex, computeDocHash } from "./doc-hash.mjs"
    ...
    if (reviewType === "design") {
      ...
      if (designToken && result && verdictPassed && code && makeApprovalCodeRegex(code).test(result)) {
        ...
        saveTokenRecord(agent.session.id, {
          ...
          // 「无法续期」处理，N7）。
          docHash: state.pendingDocHash,
          docPaths: state.pendingDocPaths,
      ...
      // FR-5（D-i / D-34）：撤销**收紧** —— 仅明确的否定裁决才撤销，且**永不静默**：
      //   ① verdict 显式 FAIL                → 撤销 + 诊断；
      //   ② 无 verdict 行 且 回落判定为不通过  → 撤销 + 诊断（含「未给出 VERDICT 行」）← D-33 情形；
      //   ③ verdict PASS 但回显缺失/不符       → **只诊断、不撤销**：回显是**签发时**的门禁，
      //      不构成作废**已签发**凭证的理由（一次「忘带回显」不该升级为丢掉已有授权）；
      //   ④ 成功签发之后的后续轮               → 不撤销（链已闭合，令牌继续有效至过期/续期）。
      // 收紧前是 `if (completed)`：它覆盖**每一个完成但未签发的 design 轮**，含 ③ 与 ④ ——
      // 一次后续轮的启发式 miss 就能销毁一枚先前已签发的有效令牌（D-33 现场即如此）。
      const explicitFail = verdict.kind === "fail"
      const heuristicFail = verdict.kind === "absent" && !isApprovalVerdict(result)
      if (completed && (explicitFail || heuristicFail)) {
        ...
        console.warn("[thincoder-suite] advisor design token 撤销（"
          + (explicitFail ? "VERDICT: FAIL" : "无 VERDICT 行且回落判定为不通过")
          + "，session " + agent.session.id + "）——磁盘记录"
          ...
          : verdict.kind === "pass" ? (hasUnresolvedBlockingRow(result) ? VERDICT_DIAG.contradiction : VERDICT_DIAG.echo)
            : verdict.kind === "fail" ? VERDICT_DIAG.fail
              : verdict.kind === "invalid" ? verdictInvalidDiag(verdict.reason)
                ...
                : isApprovalVerdict(result) ? VERDICT_DIAG.echo : VERDICT_DIAG.absentFail
