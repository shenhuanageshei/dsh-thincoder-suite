...
function renderStagesBlock(stages) {
  ...
  if (!anyHost) {
    ...
    lines.push(
      ...
      "验证命令不归你执行（本阶段不要求你执行命令）：凡标注「由宿主执行」的 Self-check 命令，由宿主在交付后执行并把验收回执写进交付报告——你只负责把命令原样报回（报告的 check summary 列），不要执行任何验证/测试命令。A stage whose edits you could not complete is failed in your table; if a stage still cannot be completed after a second genuine fix attempt, STOP: never advance a failing stage.",
  ...
  for (let i = 0; i < stages.length; i++) {
    ...
    lines.push(st.check)
    if (effMode(st) === "host") {
      // 注记放在命令行**之后**（宿主执行验收；子代理侧保持「Self-check:\n<命令>」的既有四段形态）
      lines.push("验证命令（由宿主执行，你只负责原样报回）——本阶段不要求你执行命令；宿主将在交付后执行该命令并把验收回执写入交付报告。")
    }
    lines.push("")
  }
  return lines
            ...
            run: (handle) => {
              const done = runCodexTask(deps, {
                ...
                if (env.ok) {
                  ...
                  return jobOutcome(handle, { status: "completed", detail: "eng_coder delivery", output: stageGateNote(stages, env.text) + deliveryText(env.text) + hostReceipt })
                }
                return jobOutcome(handle, { status: "failed", detail: "codex-cli " + env.code, output: failStop(failureText(env)) })
              }, (e) => {
                clearInFlightJob(sid, "eng")
            ...
            const done = (async () => {
              ...
              try {
                if (!ctx?.subagents || typeof ctx.subagents.start !== "function") {
                  return jobOutcome(handle, { status: "failed", detail: "ctx.subagents unavailable", output: failStop(warnPrefix() + "eng_coder failed: ctx.subagents is unavailable inside background job (cannot recover after dispatch)") })
                }
                ...
                return jobOutcome(handle, { status: "completed", detail: "eng_coder delivery", output: stageGateNote(stages, outputText) + warnPrefix() + "eng_coder delivery:\n" + (outputText || "(empty report)") + "\n\nNext (automatic flow nodes): verify the delivery against the acceptance criteria, run the divergence audit if this is the FIRST delivery, then run advisor(type='code', documents=[Docs involved])." + hostReceipt })
              } catch (e) {
                ...
                if (timedOut) return jobOutcome(handle, { status: "failed", detail: "dshBackgroundTimeoutMs backstop", output: failStop(warnPrefix() + "eng_coder ended: dsh 后台子代理超兜底截止（dshBackgroundTimeoutMs=" + backstopMs + ")到点，子代理已 abort—— ABORTED" + codexFailureAdvisory({ text: "", code: "ABORTED" })) + stageCheckCommandsBlock(stages) + DSH_SANDBOX_FORENSICS })
                ...
                return jobOutcome(handle, { status: "failed", detail: String(e?.message ?? e), output: failStop(warnPrefix() + "eng_coder failed: " + (e?.message ?? String(e))) })
              } finally {
