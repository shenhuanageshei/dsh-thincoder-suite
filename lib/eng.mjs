 ...
 *  `medium` 的枚举依据（会诊 v4-pro 提供，父侧复核形式）：`low` 是**唯一在一切非退化支持集上
 *  都可能被 `nearestEffort` 静默落到 `off`** 的程度档——本部署实测形状 `{off,high,max}` 恰是
 *  它的中招形状；`medium` 只在退化集 `{off}` 落 `off`。改这一个常量即同时改**默认值**与
 *  **回落目标**（P2 的隐式耦合由此消除）。 */
...
export async function runEngCoder(deps, args) {
  ...
  // 落点 = **dsh 同步路径、spawn 前**。**仅同步路径**：`args.background === true` 且 jobs 可用时
  // 已在上方 return（后台无平台墙钟问题 ⇒ 告警即错告），落到这里的两条 background 分支都是
