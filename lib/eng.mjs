 ...
 * 名字与 export，因为 `guard-e.test.mjs` 的 `WRITE_GATE_FIXTURE` 逐字节锁着
...
export async function runEngCoder(deps, args) {
  ...
  if (cliCfg.engCoderRunner === "codex-cli") {
    ...
    try {
      ...
      if (signal?.aborted) {
        ...
        return warnPrefix() + deathLine(annotateAbort(new Error("eng_coder aborted."), signal, "settle", "caller signal abort"), signal) + capNote
      }
              ...
              try {
                ...
                if (ctrl.signal.aborted || e?.name === "AbortError") {
                  ...
                  return {
                    ...
                    output: warnPrefix() + deathLine(annotateAbort(new Error("eng_coder aborted."), src, layer, "dsh subagent abort"), src),
                  }
                }
  ...
  try {
    ...
    if (signal?.aborted || e?.name === "AbortError") {
      ...
      return warnPrefix() + deathLine(annotateAbort(new Error("eng_coder aborted."), src, layer, "dsh subagent abort"), src)
    }
