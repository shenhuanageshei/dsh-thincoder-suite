...
export function engineeringToggle(agent, action, opts) {
  // Compatibility shim for older direct callers while removing the unused config parameter.
          ...
          run: () => {
            ...
            const done = (async () => {
              ...
              try {
                ...
                if (timedOut) return { status: "failed", detail: "dshBackgroundTimeoutMs backstop", output: warnPrefix() + "eng_coder ended: dsh 后台子代理超兜底截止（dshBackgroundTimeoutMs=" + backstopMs + ")到点，子代理已 abort—— ABORTED" }
