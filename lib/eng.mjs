...
import { validateDesignToken, EFFORT_LEVELS, tokenExpiryMs, resolveCodexBudgetCapMs, getJobsService, codexJobsDispatchReply, checkInFlightJob, setInFlightJob, clearInFlightJob, bumpAdvisorGeneration, sessionStateViewWithGeneration } from "./advisor.mjs"
...
export async function runEngCoder(deps, args) {
  ...
  if (cliCfg.engCoderRunner === "codex-cli") {
    const globals = resolveCodexCliGlobals(config).globals
    const runner = {
      kind: "codex-cli",
    ...
    const baseTimeout = Number.isInteger(cliCfg.defaultTimeoutMs) && cliCfg.defaultTimeoutMs >= 30000 && cliCfg.defaultTimeoutMs <= 3600000
      ? cliCfg.defaultTimeoutMs : 1800000
    const idleTimeoutMs = Number.isInteger(cliCfg.idleTimeoutMs) && cliCfg.idleTimeoutMs >= 15000 && cliCfg.idleTimeoutMs <= 3600000
      ? cliCfg.idleTimeoutMs : 300000

    // —— D-20 交付簿记闭包（同步/后台两路同语义：仅成功分支恰好一次）——
    const deliverBookkeeping = (outputText) => {
      state.advisorRound = 0
      state.lastAdvisorOutput = null
      // R2 §4.4（D-10）：eng 交付重置 = 语义转换点，代际 +1——此后晚到的在飞 advisor 结果
      // 经 finalize 代际校验丢弃，不复活已被重置的轮次/prior
      ...
      // F12（§2.3 写点表）：交付后写盘（同 dsh 路径语义；后台 done 内同样落盘）
    ...
    if (baseTimeout > budgetCap) {
      ...
      if (jobs && typeof jobs.start === "function") {
        ...
        try {
          ...
          return codexJobsDispatchReply({ prefix: "eng_coder (codex-cli)", jobId: started, budgetMs: baseTimeout, budgetCapMs: budgetCap, detail: runner.model ?? "codex 默认模型" })
        } catch (e) {
          console.warn("[thincoder-suite] ctx.jobs 派发失败（" + (e?.message ?? String(e)) + "）——eng_coder 回落同步执行（预算按 budgetCap 截断）")
        }
      } else {
        console.warn("[thincoder-suite] ctx.jobs 不可用——eng_coder codex 预算 " + baseTimeout + "ms 截为 "
  ...
  // mergeChildMutations 等价物（D-20：仅成功分支）：子交付 → 重置 advisor 预算（交付 code review 必然发生）
  state.advisorRound = 0
  state.lastAdvisorOutput = null
  bumpAdvisorGeneration(state) // R2 §4.4（D-10）：eng 交付重置 = 语义转换点，代际 +1（在飞 advisor 结果晚到即弃）
  const touched = parseTouchedFiles(outputText)
  if (touched.length > 0) {
    const merged = new Set([...(state.touchedFiles ?? []), ...touched])
    state.touchedFiles = [...merged]
