...
export async function runEngCoder(deps, args) {
  ...
  // —— F9-2（评审反馈）：engCoderEffort 按目标模型实际支持档位校验/回落（共享 helper
  // effort-resolve.mjs，四个 effort 消费点同修；"low" 对 glm-5.3-flash 会
  // UNSUPPORTED_REASONING_EFFORT 秒死——2026-09-04 eng_coder 三连败根因）。 —————————————
  ...
  if (cliCfg.engCoderRunner === "codex-cli") {
    ...
    const runner = {
      ...
      effort: engCoderEffort === "off" ? null : engCoderEffort,
    }
    try {
      const env = await runCodexTask(deps, {
        ...
        timeoutMs: Number.isInteger(cliCfg.defaultTimeoutMs) ? cliCfg.defaultTimeoutMs : 1800000, // 8.4：长任务默认放大
        idleTimeoutMs: Number.isInteger(cliCfg.idleTimeoutMs) ? cliCfg.idleTimeoutMs : 300000,
        runner,
        config,
        signal,
      })
      const outputText = env.text || ""
      const touched = parseTouchedFiles(outputText)
      // mergeChildMutations 等价物（与 dsh 路径同语义）：交付 → 重置 advisor 预算 + touched 并入
      ...
      if (env.ok && env.text) {
        return warnPrefix() + "eng_coder delivery (codex-cli):\n" + outputText +
      ...
      return warnPrefix() + "eng_coder ended: codex-cli " + env.code + " — " + (env.userMessage || env.diagnostics || "unknown") + partial
    } catch (e) {
      if (signal?.aborted) return warnPrefix() + "eng_coder aborted."
