...
- The eng_coder call itself is rejected without a valid design token — that spawn-time check IS the enforcement.
...
- Write code one file at a time, verify each before moving on: run a syntax check after each edit and the relevant tests after each logical group.
