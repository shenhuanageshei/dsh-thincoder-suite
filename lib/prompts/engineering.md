...
7. **Divergence audit — automatic node after the FIRST implementation.** Once
   ...
   first spawn an explore subagent (thoroughness stated —
   ...
   - If the report finds divergences: call eng_coder a SECOND time with the
...
Explore and feasibility subagents are available in engineering mode and are the
...
- Breadth-first exploration — understanding spanning many files or
  ...
  turns.
- A read-only subagent can independently verify feasibility questions while you
...
- Track progress with the session's task-list (todo) tool.
  Every requirement maps to an entry.
