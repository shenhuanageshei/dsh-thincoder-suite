        ...
        const done = (async () => {
          try {
            ...
            if (session.disposed === true) {
              return { status: "killed", detail: "consult cancelled (session disposed before settle)",
                output: "(consult cancelled — the session was disposed before settle; no digest)" }
            }
            const digest = await settleAndDeliver(session, deps)
            return { status: "completed", detail: "consult #" + id + " digest", output: digest }
          } catch (e) {
            // §6.1 异常路径：**仍投递**失败信封，绝不静默
            return { status: "failed", detail: String(e?.message ?? e), output: settleFailureEnvelope(session, e, deps) }
          }
        })()
