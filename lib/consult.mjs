...
function settleChild(session, id, modelLabel, ok, payload) {
  if (ok) {
    ...
    // 自己的结算面**（consult_check 的 reply 队列），与 failed 面同一队列、同一读取协议：
    // D-AP6 要求死亡行进 output 尾部、US-1 要求「报告尾部含正确的 abort(...)」——早停/销毁也要自证。
...
export function cleanupConsultSessions(state) {
  for (const s of state.consultSessions?.values() ?? []) {
    ...
    for (const c of s.controllers ?? []) { try { c.abort(abortError(null, "settle", "session disposed", "cancel")) } catch { /* noop */ } }
