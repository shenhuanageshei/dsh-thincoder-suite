      ...
      run: (handle) => {
        const done = (async () => {
          try {
            ...
            return jobOutcome(handle, { status: "completed", detail: "consult #" + id + " digest", output: digest })
          } catch (e) {
            // §6.1 异常路径：**仍投递**失败信封，绝不静默
            return jobOutcome(handle, { status: "failed", detail: String(e?.message ?? e), output: settleFailureEnvelope(session, e, deps) })
          }
        })()
