...
export function consultDigestionGate(state, ack, deps) {
  ...
  const lines = [
    ...
    "消化 = ① `job_output` 读全文 ② 逐条处置（采纳 / 不采纳**附理由** / 待定）③ 写纪要裁定层（§2–§5）④ ack 回报。",
  ]
  for (const s of open) {
    lines.push("")
          ...
          try {
            ...
            if (session.disposed === true) {
              ...
              return jobOutcome(handle, { status: "killed", detail: "consult cancelled (session disposed before settle)",
                output: "(consult cancelled — the session was disposed before settle; no digest)" }, { pathForm: "consult-minutes" })
            }
            const digest = await settleAndDeliver(session, deps)
            return jobOutcome(handle, { status: "completed", detail: "consult #" + id + " digest", output: digest }, { pathForm: "consult-minutes" })
          } catch (e) {
            // §6.1 异常路径：**仍投递**失败信封，绝不静默
            return jobOutcome(handle, { status: "failed", detail: String(e?.message ?? e), output: settleFailureEnvelope(session, e, deps) }, { pathForm: "consult-minutes" })
          }
        })()
