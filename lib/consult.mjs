...
import { normalizeRunnerValue, runCodexTask, codexRowLabel } from "./codex-adapter.mjs"
...
async function startConsultChild(deps, session, m, problem, signal, historyText) {
  ...
  if (nr.ok && nr.runner.kind === "codex-cli") {
    ...
    try {
      ...
      if (env.ok && env.text) settleChild(session, null, codexLabel, true, env.text)
      else if (env.code === "ABORTED") settleChild(session, null, codexLabel, false, "aborted")
      else settleChild(session, null, codexLabel, false, "codex-cli " + env.code + ": " + (env.userMessage || env.diagnostics || "failed"))
    } catch (e) {
