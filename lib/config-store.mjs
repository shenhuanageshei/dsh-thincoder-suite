...
//     consultTimeoutMs（顶层全局，0.9.3 用户层化——见下方常量块注释）；
//   未知字段不合并（白名单）；base 的非白名单键（engineering/engTokenTtlMs/
...
// ————————————— 0.9.3：consult 看门狗超时（consultTimeoutMs）用户层化 —————————————
// 会诊子代理（dsh 行 + codex 行共用）的看门狗预算。原为 entry base 专属键（cordis.patch.yml
// 默认 600000）——base 是启动快照且插件更新整包覆盖，用户改不动、升级即丢（2026-09-09 生产
// 反馈：deepseek-v4-pro/max 在超大会话下偶发超过 10 分钟看门狗被杀，无法调大）。0.9.3 起按
// dshBackgroundTimeoutMs（R5 D-27）同款三面收口：PUT 校验（index.mjs）⊕ merge 白名单（本文件
// mergeGlobalConfig）⊕ 运行时解析（本文件 resolveConsultTimeoutMs，consult.mjs 消费）。
// 缺省 600000（10min）；合法 60000..3600000。
export const CONSULT_TIMEOUT_MIN_MS = 60_000
export const CONSULT_TIMEOUT_MAX_MS = 3_600_000
export const CONSULT_TIMEOUT_DEFAULT_MS = 600_000

/** consultTimeoutMs 只认 60000..3600000 的整数（PUT 白名单面与运行时回落共用单一事实源）。 */
 ...
 * 未配 → 缺省 600_000（10min）；非法 → 回落缺省并 console.warn 响亮告警（N4 先例）。
 * 运行时宽容正整数值域（对齐 resolveDshBackgroundTimeoutMs 先例）：正整数值即生效——测试/手编
 * 小值可驱动看门狗（consult.test 的 25ms 用例依赖此语义）；PUT 面仍按 60000..3600000 收口。
 * @param {object} config — 生效全局配置（effectiveGlobalConfig 输出）
 ...
 * - consultTimeoutMs（0.9.3）：顶层全局——user 定义（非 null）即覆盖（会诊看门狗预算；
 *   运行时 resolveConsultTimeoutMs 校验/回落；原先只存在于 entry base，见常量块注释）；
 * - 未知顶层/组内字段不合并（白名单）；base 的非白名单键（engineering / engTokenTtlMs 等）
 *   原样保留——user 层只叠加白名单，不改 entry base 其他语义。
...
export function mergeGlobalConfig(baseConfig, userCfg) {
  ...
  // 0.9.3：顶层全局 consultTimeoutMs（merge 白名单面——运行时 resolveConsultTimeoutMs
  // 校验/回落；本层只做白名单透传，不重复值校验）
