...
// + run.mjs 的 extractUnfixedIssues，合并为单模块。
// 差异（诚实标注）：Project Guide 预算取固定 16K（DSH 侧无模型 spec 表，无法按窗口 5% 缩放）。
// design token 协议 v2：Approval Signal 改为展示
...
/** code 评审的判据正文（`## Review Criteria` 段的**内容**）——**三态显式，零静默**（批 13 / R-6）。
 *  键 = `advisor.criteriaDoc`——**独立键**，严禁并入 `standardsDoc` 家族：后者是**独立追加段**
 *  且两面都注入，而本键**替换判据段的内容**、仅 code round 1 消费（并键会让「声明了标准档的仓」
 *  的 code 判据被静默替换）。
 *    ① 已声明且可读 → 注入该文件内容（照抄标准段/地图段先例：**无预算**——判据注入预算登记为
 *       后续项，见设计档 §9 边界 5）
 *    ② 已声明但不可读 → **响亮句点名 `advisor.criteriaDoc`**（镜像标准段/地图段的 D-P7 风格），
 *       再回落 `DEFAULT_CRITERIA`
 *    ③ 未声明 → ★ **legacy 回退链，逐字节保持今日行为**：探 `<cwd>/.thincoder/advisor.md`，
 *       可读则注入，否则回落 `DEFAULT_CRITERIA`
 *  ★ 该链的 `.thincoder/advisor.md` 是**上游产品名路径**——批 13 保留为 legacy 回退（退役是
 *    **破坏性**的：存量部署会静默失去判据，故另立批次；见设计档 §9 边界 3）。
 *  ★ 存量用户（未声明本键）**逐字节零行为变更** = US-7 的验收面。
 */
function loadAdvisorMd(cwd, criteriaDoc) {
  if (typeof criteriaDoc === "string" && criteriaDoc.trim() !== "") {
    try {
      // 态 ① 声明且可读 —— 基与标准段/地图段同尺（cwd 缺省回落进程 cwd 只对新键生效；
      // 下面 legacy 分支的基**逐字节不动**）
      return readFileSync(resolve(cwd ?? process.cwd(), criteriaDoc), "utf8")
    } catch {
      // 态 ② 声明但不可读（含路径不可解析）—— **响亮句点名配置键** + 回落内置判据
      return "(Declared review criteria document '" + criteriaDoc + "' could not be read — check the advisor.criteriaDoc setting. Falling back to the built-in criteria.)\n"
        + DEFAULT_CRITERIA
    }
  }
  // 态 ③ legacy 回退链（★ 逐字节保持今日行为；`.thincoder/advisor.md` = 上游产品名路径）
  try {
...
export function buildAdvisorUserMessage(opts) {
  ...
  // opts.standardsDoc / opts.documentMapDoc / opts.criteriaDoc = 生效全局配置里声明的**三个**
  // 文档路径（可缺省；criteriaDoc 是批 13 / R-6 新增的「code 评审判据正文」键）。
  const { cwd, history, state, reviewType, designToken, documents, paths, standardsDoc, documentMapDoc, criteriaDoc } = opts
  ...
  if (!hasPrior) {
    ...
    parts.push(loadAdvisorMd(cwd, criteriaDoc))
    if (guideRoot) {
      parts.push("")
