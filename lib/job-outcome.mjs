...
/**
 * 测试缝：清零上面的「只 warn 一次」闩。
 * 背景（批 29 / US-4 🔵③）：A27-4 的「只 warn 一次」判据此前**依赖用例顺序**——只有本进程里
 * 第一个触发 no-home 形态的用例才能看到首条告警；把该腿挪个位置它就会红（断言测的是「顺序」
 * 而不是「闩」）。有了本缝，用例自带前置 ⇒ **顺序无关**。
 *
 * ★ 可见性机制（设计 §2.4 评审 #8）：本仓无私有导出 ⇒ 以**命名约定 + JSDoc `@internal`** 为准
 *   —— `__` 前缀 + `ForTest` 后缀 + 本标注，三者同时在场才构成「测试缝」的可见标记。
 * ★ 生产路径零引用：`lib/**` 内除本档自身外的任何模块引用本缝，都会让
 *   `test/job-persistence.test.mjs` 的静态锁转红（防测试缝被顺手接进生产路径）。
 * @internal 仅测试使用；不属于插件对外契约，可随时改名/删除。
 */
export function __resetWarnedNoHomeForTest() { warnedNoHome = false }
 ...
 *   maxAgeDays：超龄阈值（天，默认 7）——**必须 > 0**（批 29 / US-4 🔵①）：`0` 属**非法值**，
 *   与「非有限数」一样回落到缺省 7。为什么拒绝 0：`0` 的语义是 `cutoff = now` ⇒ **见到的作业
 *   文件全删**（连同索引一起清空整个 jobs 目录），而它与「正常轮转」共用同一个数值域 ⇒ 一次
 *   手滑（少写一位 / 把 0 误当「不限龄」）就会造成**不可逆的批量删除**。本清扫的定位是**尽力
 *   而为的轮转**（D27-1），不该存在任何「一参数清空」的入口 ⇒ 取严格正数；确需清空时由人直接
 *   操作文件系统（有意、可见、可复核）。相邻的 `maxFiles: 0`（保留 0 个）语义相反，但**不在
 *   本单范围内**——本单只收 `maxAgeDays`（改动面最小化）。
 *   maxFiles：保留条数上限（默认 200）·
...
export function sweepJobReports(opts) {
  ...
  try {
    // 批 29 / US-4 🔵①：校验收紧为 **> 0**（原 `>= 0`）——`0` = 「一参数清空 jobs 目录」，
    // 显式拒绝并回落缺省 7（为什么拒 0 见本函数 JSDoc）。
    ...
    for (const name of names) {
      ...
      let mtimeMs
      // 批 29 / US-4 🔵②：stat 失败（目录 / 占用 / 权限）⇒ 该条目**跳过**，故它**不计入 `kept`**
      // ——摘要的 kept = 「可 stat 且存活」的条数，**不是**「目录里形态命中的条数」；如实登记。
