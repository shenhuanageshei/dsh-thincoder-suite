...
/** envelope 错误码（研究文档 §5.4）。 */
export const CODEX_ERROR_CODES = ["OK", "RUNNER_UNAVAILABLE", "PROCESS_ERROR", "NO_OUTPUT", "PROTOCOL_ERROR", "TIMEOUT", "ABORTED", "CLEANUP_FAILED"]
...
/** codexCli 全局默认（D1：agentsMdPolicy 默认 disable）。 */
...
/** agentsMd=clean-cwd 的工作目录（无 cleanCwdRoot 时一次性临时目录——评审 #2：目录句柄\n *  随调用返回，fin 统一清理；创建失败回落原 cwd）。 */
function resolveChildCwd(cwd, policy, globals) {
  if (policy !== "clean-cwd") return { cwd, tempDir: null }
  if (globals.cleanCwdRoot && existsSync(globals.cleanCwdRoot)) return { cwd: globals.cleanCwdRoot, tempDir: null }
  try {
...
export async function runCodexTask(deps, opts) {
  ...
  const cc = resolveChildCwd(opts.cwd ?? process.cwd(), agentsMd, globals)
  ...
  return await new Promise((resolveDone) => {
        ...
        if (Date.now() - lastActivityAt > idleTimeoutMs) {
          ...
          killTree(deps, child).then((ok) => {
            ...
            fin(emptyEnvelope("TIMEOUT", {
              diagnostics: "idle " + idleTimeoutMs + "ms 无输出事件，判定假死并终止进程树" + (stderr ? " | stderr: " + stderr.slice(0, DIAGNOSTICS_MAX) : ""),
    ...
    // B9：watchdog（wall clock；idle 活性信号 = 二期 T2.3）
    const watchdog = setTimeout(async () => {
      terminating = true
      killFailed = !(await killTree(deps, child))
      fin(emptyEnvelope("TIMEOUT", {
        diagnostics: "timeoutMs=" + timeoutMs + " 到期，已终止进程树" + (stderr ? " | stderr: " + stderr.slice(0, DIAGNOSTICS_MAX) : ""),
        text: lastAgentText.slice(0, 4000),
        threadId,
        runner: { kind: "codex-cli", version: probeVersion },
      }))
    ...
    child.stderr?.on?.("data", (d) => {
      touchActivity()
      if (stderr.length < DIAGNOSTICS_MAX) stderr += String(d)
    })
    ...
    child.on("exit", (code) => {
      ...
      const diag = stderr ? "stderr: " + stderr.slice(0, DIAGNOSTICS_MAX) : ""
      if (code === 0) {
      ...
      const env = emptyEnvelope("PROCESS_ERROR", { ...base, diagnostics: "exit=" + code + (diag ? " | " + diag : ""), text })
