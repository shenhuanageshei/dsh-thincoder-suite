...
/** profile 根特征 = 目录同时含 sessions/ 与 settings.yaml（设计 §5 确认项 1）。 */
...
export function saveTokenRecord(sessionId, entry, dshHomeOverride, cwdHint) {
  ...
  try {
    // 1. 确保目录存在（评审 #2：目录缺失时 writeFile ENOENT 会静默失败）
    ...
    // 5. 写回（失败仅 warn 不抛——签发不依赖持久化成功）
    writeFileSync(filePath, JSON.stringify({ version: STORE_VERSION, tokens }, null, 2) + "\n", "utf8")
    return true
  } catch (e) {
