...
export function validateGlobalUserConfig(userConfig, knownProviders) {
  ...
  if (userConfig.advisor !== undefined && userConfig.advisor !== null) {
    ...
    if (typeof adv !== "object" || Array.isArray(adv)) {
      ...
      const advUnknown = Object.keys(adv).filter((k) => ![...GROUP_KEYS, "includeProjectGuide", "maxOutputTokens", "contextTokens"].includes(k))
      for (const k of advUnknown) errors.push("advisor." + k + " is not supported (expected " + GROUP_KEYS.join("|") + "|includeProjectGuide|maxOutputTokens|contextTokens)")
      const advOut = {}
