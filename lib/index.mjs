  ...
  ctx.effect(() => {
    ...
    register(textTool({
      ...
      description:
        ...
        "The dsh sub-agent path runs as a BACKGROUND job by default (omit background or pass true; pass false to force synchronous execution): the tool return is a job handle, NOT the delivery — wait for the completion notice, then read the FULL report with job_output (the stage status table and stage-gate banner live in the job output), verify it, and run the delivery review (advisor type='code').",
      parameters: {
        type: "object",
        properties: {
          ...
          background: { type: "boolean", description: "dsh 子代理路径默认后台执行（返回 job 句柄，等完成通知后用 job_output 读全文交付报告）；传 false 强制同步（内部截止 codexCli.budgetCapMs，受平台墙钟约束）" },
