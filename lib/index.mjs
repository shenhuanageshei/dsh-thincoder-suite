...
export function validateGlobalUserConfig(userConfig, knownProviders) {
  ...
  if (userConfig.advisor !== undefined && userConfig.advisor !== null) {
    ...
    if (typeof adv !== "object" || Array.isArray(adv)) {
      ...
      for (const k of advUnknown) errors.push("advisor." + k + " is not supported (expected " + GROUP_KEYS.join("|") + "|includeProjectGuide|maxOutputTokens|contextTokens|standardsDoc|documentMapDoc)")
      const advOut = {}
      ...
      // 批 7（D-P4/J11）：PUT 白名单面——advisor.standardsDoc / advisor.documentMapDoc。
      // 语义 = 「项目标准文档 / 文档地图在哪」（项目属性，global-only）。路径必须**是文档**：
      // 判据复用**同一个单一权威** isDocPath（与写门禁同尺，J11）；这里是**类型校验**不是
      // 可读性校验（存在吗）——可读性在注入时判（PUT 层没有会话 cwd，无法可靠解析相对路径）。
