...
import { sessionState, dropSession, dropAllSessions, engEffective, persistedSessionView, restoreSessionState } from "./state.mjs"
import {
...
import { engineeringToggle, makeWriteGate, runEngCoder, attachEngineeringSection } from "./eng.mjs"
...
import { validateRunnerValue, normalizeRunnerValue, resolveCodexCliGlobals, discoverCodexModels } from "./codex-adapter.mjs"
...
export function validateGlobalUserConfig(userConfig, knownProviders) {
  const errors = []
  const notes = []
  const topAllowed = ["advisor", "consultModels", "engCoderMaxTokens", "engCoderEffort", "codexCli"]
  ...
  if (userConfig.advisor !== undefined && userConfig.advisor !== null) {
    ...
    if (typeof adv !== "object" || Array.isArray(adv)) {
      ...
      const advUnknown = Object.keys(adv).filter((k) => ![...GROUP_KEYS, "includeProjectGuide", "maxOutputTokens"].includes(k))
      for (const k of advUnknown) errors.push("advisor." + k + " is not supported (expected " + GROUP_KEYS.join("|") + "|includeProjectGuide|maxOutputTokens)")
      const advOut = {}
  ...
  if (userConfig.codexCli !== undefined && userConfig.codexCli !== null) {
    ...
    if (typeof cc !== "object" || Array.isArray(cc)) {
      ...
      const ccFields = ["executable", "proxyMode", "proxyUrl", "agentsMdPolicy", "model", "defaultTimeoutMs", "idleTimeoutMs", "engCoderRunner", "budgetCapMs"]
 ...
 * 假阳性（settings 文件里根本没有内置适配器）。两侧都不可读 → undefined（跳过存在性校验）。
 */
  ...
  return async (req, res) => {
    ...
    try {
      ...
      if (req.method === "GET" && path === "/catalog") {
        ...
        if (!llm || typeof llm.listProviders !== "function") {
          return send({ ok: false, error: "llm runtime 不可用（listProviders 缺失）" }, 502)
        }
  ...
  ctx.effect(() => {
      ...
      async execute(args, exec) {
        ...
        if (typeof out === "string" && /^advisor_config: (set|reset) /.test(out)) {
          try {
            saveSessionState(
              agent.session.id,
              persistedSessionView(agent.session.id),
              undefined,
    ...
    register(textTool({
      ...
      description:
        ...
        "If a stage fails, re-spawn ONE new eng_coder with corrected stages starting from the failed stage (the token is not consumed). On return, run the delivery review (advisor type='code').",
      parameters: {
        type: "object",
        properties: {
          ...
          stages: {
            ...
            items: {
              ...
              properties: {
                ...
                check: { type: "string", minLength: 1, description: "Self-check command (e.g. node --check lib/x.mjs, or node --test on a subset)" },
              },
    ...
    if (pool.length > 0) {
      register(textTool({
        ...
        async execute(args, exec) {
          ...
          const out = await runEscalate({ ctx, agent, config: effectiveGlobalConfig(config), state: sessionState(agent.session.id), signal: exec?.signal, configDefaultEngineering }, args?.task, args?.model, args?.followup === true)
    ...
    const apiDispose = registerConfigApi(ctx, {
      ...
      // persistedSessionView 组装视图，saveSessionState fail-safe 仅 warn 不抛）。
      ...
      persistSession: (id) => {
        ...
        saveSessionState(String(id), persistedSessionView(String(id)), undefined, targetCwd ?? cwdHintOf(ctx))
      },
    })
