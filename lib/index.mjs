    ...
    if (pool.length > 0) {
      ...
      register(textTool({
        ...
        description:
          ...
          // 批 14（A 项）：补「无通知、必须自己回来」的硬约束——本协议是「本回合 start、后续回合 check」的**跨回合协议**
          // （见 lib/consult.mjs 档头 D-28），而 consult **不走 ctx.jobs** ⇒ 平台不会推完成通知。历史上调用方
          // 发起后结束回合、等通知，于是协议永不推进。其余三机制（advisor/eng_coder/escalate）走 jobs.start 有通知，此处无。
          "★ There is NO completion notification: consult does not run as a platform job (unlike advisor / eng_coder / escalate). " +
          "DO NOT end your turn after consult_start expecting to be woken — you must come back yourself. " +
      ...
      register(textTool({
        ...
        description:
          ...
          "★ Blocking is the whole point: this call IS how you wait — there is no notification to wait for. Call it immediately after consult_start, in the same turn. " +
      ...
      register(textTool({
        ...
        description:
          ...
          "Use it also when you must leave the consult unattended: since consult has no completion notification, a consult left running is only ever read if you come back.",
        parameters: {
          type: "object",
          properties: {
            id: { type: "string", description: "Consult id from consult_start" },
