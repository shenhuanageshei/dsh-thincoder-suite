...
import {
  ...
  // 0.9.3：consultTimeoutMs 用户层化（PUT 校验面——merge/运行时在 config-store.mjs）
  CONSULT_TIMEOUT_MIN_MS, CONSULT_TIMEOUT_MAX_MS,
  isValidConsultTimeoutMs,
...
export function validateGlobalUserConfig(userConfig, knownProviders) {
  const errors = []
  const notes = []
  const topAllowed = ["advisor", "consultModels", "engCoderMaxTokens", "engCoderEffort", "codexCli", "dshBackgroundTimeoutMs", "consultTimeoutMs"]
  ...
  // 0.9.3：顶层全局 consultTimeoutMs（会诊看门狗）——PUT 白名单面（merge 白名单在
  // config-store.mjs、运行时回落告警在 resolveConsultTimeoutMs、消费在 consult.mjs——单一事实源）
      ...
      register(textTool({
        ...
        async execute(args, exec) {
          ...
          const r = await startConsultSession(
            ...
            // run_code 程序 settle 时被 abort，跨回合协议永远读不到回复）
