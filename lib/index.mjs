    ...
    if (pool.length > 0) {
      register(textTool({
        ...
        async execute(args, exec) {
          ...
          const out = await runEscalate({ ctx, agent, config: effectiveGlobalConfig(config), state: sessionState(agent.session.id), signal: exec?.signal, configDefaultEngineering }, args?.task, args?.model)
