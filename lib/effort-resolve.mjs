...
// model-a-flash 不受支持（reasoningEfforts 仅 off/high/max），pi-ai
...
// 批 19（2026-09-17 · 2026-09-17-effort-off-fallback-design.md）：off 是关闭开关不是力度档
// ⇒ 不参与距离竞争——回落只在力度域内进行；退化兜底：模型在插件梯子上除 off 外无力度档时，
// 回落到唯一可执行值 off（专属告警「退化兜底」）；显式请求 off 而模型不支持 ⇒ 省略
// reasoningEffort（effort: null，交还提供方默认）+ 专属告警（「未能关闭推理」），不悄悄换档。
...
/** dsh 梯子上的「关闭推理」开关档名——**不是力度档**（批 19 / D19-1：退出距离竞争，仅退化兜底用）。 */
const DSH_EFFORT_OFF = "off"
 ...
 *          note 非空 = 发生了回落/退化兜底/显式 off 关不掉，或透传（元数据不可得）——三条措辞
 *          互斥分叉（批 19 / D19-4）；调用方应把告警带进结果尾部（附录 D.3 规则）+ 本函数已
 *          console.warn 响亮留档。
 */
export async function resolveSupportedEffort(llm, provider, model, requested) {
  ...
  // 批 19（FR-2 / D19-3）：显式请求 off 而模型不支持 ⇒ 省略 reasoningEffort（effort: null，
  // 交还提供方默认档）+ 专属告警。对称性只转移值（null），不转移沉默（会诊 [2]-3 / 纪要 §3 D-3）；
  // 与 codex 侧 resolveCodexRowEffort 的 off⇒null（无条件改名、静默，R-58 有意不解钉）语义不同源。
  if (requested === DSH_EFFORT_OFF) {
    const note = "effort \"off\" is not supported by model " + model
      + " (supported: " + efforts.join("|") + ") — 未能关闭推理：已省略 reasoningEffort（交还提供方默认档）"
    WARN(note)
    return { effort: null, note }
  }
  // 批 19（FR-1 / D19-1）：候选池分层——off 是关闭开关不是力度档，退出距离竞争。按名排除
  // （!== DSH_EFFORT_OFF；禁按梯子下标——codex 梯子 [0] 是 low，按下标会真打穿 codex）。
  // 退化判定 = degrees 空且 cands 非空（D19-2；**不是**「nearest 返回 null」——那会把 {off,xhigh}
  // 判成非退化 ⇒ 透传一个已知不被支持的值 ⇒ 秒死路径，且差集从 3 组扩大）。
  const cands = efforts.filter((e) => DSH_EFFORT_LADDER.includes(e))
  const degrees = cands.filter((e) => e !== DSH_EFFORT_OFF)
  const degenerate = degrees.length === 0 && cands.length > 0
  const nearest = nearestEffort(requested, DSH_EFFORT_LADDER, degrees.length > 0 ? degrees : cands)
  if (nearest) {
    // 批 19（FR-3 / D19-4）：三条 note 措辞分叉。普通回落句逐字保留（既有断言依赖）；
    // 退化兜底的**值**与改前相同 ⇒ 判别力全在专属子串「退化兜底」上，绝不得复用 falling back 句。
    const note = degenerate
      ? "effort " + JSON.stringify(requested) + " is not supported by model " + model
        + " (supported: " + efforts.join("|") + ") — 退化兜底：该模型在插件梯子上除 off 外无力度档，"
        + "已回落到唯一可执行档 " + JSON.stringify(nearest) + "（off 仅在退化兜底时作为候选）"
      : "effort " + JSON.stringify(requested) + " is not supported by model " + model
