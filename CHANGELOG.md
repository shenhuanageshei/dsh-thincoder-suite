...
> **一句话**：**把门禁变可信**——让子代理**根本跑不了**验证命令（工具面禁执行）、卡住时**静默 5 分钟就掐**（不再白等 60 分钟）、把 `check` 的语义从「子代理自检」正名为「宿主验收清单」；顺手把**阶段门那条恒假阳性**（每次交付都喊 UNDECLARED）修成真谓词；并收掉批 27/28 交付码评留下的 7 条 🔵 与会诊只读白名单的锁缺口。**本版改 `lib/**` ⇒ 重启 DSH 后生效**。

**批 29（把门禁变可信：D-50 · D-51 · D-52 · D-54）**

- **【计数行】**：`node --test` **584/584**（**基线 567 + 本批 17**：新档 `test/silence-watchdog.test.mjs` +8 · 新档 `test/repro-script-readonly.test.mjs` +2 · `test/stage-gate.test.mjs` 6→7 · `contract-baseline` 8→10 · `job-persistence` 9→12 · `consult` 6→7）。⇒ 台账 §三新增两行，档数 **26 → 28**。
- **D-50（🔴 流程：`eng_coder` 自跑进程内验证撞兜底）修法**：① **工具面禁执行**——`toolFilter` 只下发**与平台公开面 `view().restrictableNames` 求交后**的名字（**平台保留名 `run_code` 一命名即抛错**，已从谓词移除）；取不到该面 ⇒ `applied:false` + **裸 warn「未生效」** + deny 逐字基线（**不假装拦住**）。② **静默看门狗**——叶子模块 `lib/silence-watchdog.mjs`（`ENG_SILENCE_ABORT_MS` 缺省 300000）；心跳取自 `run()` 子代理事件面观察（**不用** `job_output` 轮询）；**首次真实事件前不武装**（零事件不误杀，退回总预算）；到点由既有取消写点中止，文案含「疑似挂死」。③ `stages[].check` **语义正名**「宿主验收清单」（host 态标签含「由宿主执行，你不要执行」；非 host 态逐字节不变）。
- **D-51（🟡 `T-AP4b` 偶发红）修法**：改**假时钟**（只替换 `Date.now`，零真实等待）⇒ 宿主**受控负载 N=20 零红**；同档其余 7 条依赖真实等待的腿**逐条点名**并只加**失败差量输出**（清单落在设计档以保证可核）。
- **D-52（🟡 会诊只读白名单无对位锁）修法**：加静态 + 行为两条对位锁（`test/consult.test.mjs`）；**`lib/consult.mjs` 零改动**（平台契约复核：`allow` = keep only、未注册名**响亮拒绝**）。
- **D-54（🟡 阶段门恒假阳性）修法**：原谓词要求**恰 3 列 + 英文 `stage`/`status`/`summary`**，而真实报告是中文表头或多一列 ⇒ **每次交付都喊 UNDECLARED**（恒假阳性 = 门不存在）。现改为 **≥3 列按名定位**（中英列名与状态词、容忍粗体/代码记号）、**保留**「无表 ⇒ UNDECLARED」与 4000 字符窗、横幅**按「无表 / 表在场但不可解析」分岔**；新增 **T-SG7 五类**用例 + 还原即红（旧谓词字面实测归零）。
- **七条 🔵 收干**（批 27/28 交付码评）：`maxAgeDays` 校验收紧为 `> 0` + JSDoc 理由 · `kept` 口径注释 · 零落盘测试缝（含生产零引用静态锁）· 键过滤化简 · `once` 占坑语义注释 · 版本词元大小写归一 · 复现脚本只读声明按 `$tempHomeOk` 分支。
- **A28-6 机验腿**：新档 `test/repro-script-readonly.test.mjs`——五条只读谓词（每条带**阳性/阴性自证**）+ 脚本声明的 if/else **结构级**断言。
- **流程实据（如实留痕）**：本批遭遇**环境侧故障**（插件 spawned-child 派发路径自 14:22 起**派发即死**；判别探针证明平台子代理面正常）⇒ 用户裁定**改道** harness subagent 实现，**程序性例外已记设计档 §7**（旁路令牌/阶段门；验收**加强**为「宿主全量 + 分歧审计 + 交付码评」三道）。两轮**只读分歧审计**分别判定「需修复轮」，并挖出 **`run_code` 保留名地雷**（一旦探测生效会打断派发）与**空集面未接线**等问题；**另一会话的独立反馈**促成 D-54 的发现。
- **残留**：**D-53**（看门狗/禁执行的**生产生效性**未验证——需一次真机派发，届时按「首派一次性留档」闭环）· `maxFiles: 0` 的同族语义（相邻参数，未在本批改动面内）· 同步快路径不装看门狗（设计口径，由 `budgetCap` 兜底）。
## [0.29.7] — 2026-09-26

> **一句话**：**对外收口**——把三条平台问题整理成**可一击复现的上游报告包**；给插件装上**版本哨兵**（平台换契约时响亮提示，而不是撞）；把九条触点矩阵收成**单一事实源**；出一份结项说明。**本版改 `lib/**` ⇒ 重启 DSH 后生效**。

**批 28（对外收口：上游报告包 ⊕ 版本哨兵 ⊕ 契约面巡检 ⊕ 结项说明）**

- **【计数行】**：`node --test` **567/567**（**基线 559 + 本批 8**：新档 `test/contract-baseline.test.mjs` +8）。⇒ 台账 §三新增一行，档数 **25 → 26**。
- **US-1 上游报告包**：`docs/upstream-2026-09-26-platform-issues.md`（三问题各含 现象/最小复现/一手证据/影响面/给上游的建议）+ 只读复现脚本 `scripts/repro-platform-issues.ps1`（零写仓、零 `git`、纯 ASCII）。
- **US-2 版本哨兵**：装配期读平台版本（**三源钉死顺序** + **来源 evidence**）并与基线对照——一致⇒静默；不一致⇒**一条响亮告警且不阻断装配**；取不到⇒标 `unknown` 也不崩（`lib/contract-baseline.mjs` + `lib/index.mjs`）。
- **US-3 契约面巡检**：**只读**工具 `contractWatch` 一次返回 `{platformVersion, versionSource, locks(9 条·三态), jobsDir}`；九条谓词**实现单源**（`test/platform-surface.test.mjs` 删净 231 行谓词体、改为 import）。
- **US-4 结项说明**：`docs/dsh017-program-closeout.md`（批次脉络 24–28 · 矩阵**只做指针 + 摘要**〔权威源 = `lib/contract-baseline.mjs`〕· 四道护栏 · 「平台换契约的标准动作」）。
- **同批上调常设锁**：批 26 锁 #7「注册面期望计数」**7 → 8**（新增 `contractWatch`；理由写在 `lib/contract-baseline.mjs` 注释内，经副本变异实测承重）。
- **流程实据**：设计评审 3 轮至 PASS（第 2 轮抓到我自己引入的两处档面矛盾）· 实现 `eng-dsh-6` 在**自检阶段**撞满 60 分钟兜底被 abort（⇒ 发现即登记 **D-50**）· 只读分歧审计判定**代码面干净**（零半截写入、零未接线、锁活性经副本变异实测为真），并**用副本复算**确认收口期的 4 条红全属登记级联 · 收口期新登记 **D-51**（`T-AP4b` 偶发红，附实测频次）。
- **残留（转批 29 候选，本批如实划出）**：**D-50**（eng_coder 自跑进程内验证撞兜底，两次同款）· **D-51**（偶发红）· **A28-6 的机验腿**（脚本只读性本批以人眼核验背书，未补静态腿）。
## [0.29.6] — 2026-09-26
...
- **【计数行】**：`node --test` **522/522**（**基线 516 + 本批 6**：D-40 = `config-api.test.mjs` +1 · D-41/D-42 = 新档 `dsh017-compat.test.mjs` +4 与 `truncation.test.mjs` +1（用例数 4 → 5））。⇒ 台账 §三：`config-api.test.mjs` **33 → 34** · `truncation.test.mjs` **4 → 5** · **新增 `dsh017-compat.test.mjs`（4）**；档数 **21 → 22**。
...
- **★ 本批改 `lib/**` ⇒ 重启 DSH 后生效。**
- **【评审】**：`advisor type=code` 对 `lib/job-owner.mjs` —— **`VERDICT: PASS`**（"No correctness, security, resource, or race issues found"）。它自己 grep 了全部 7 个派发点并逐个给出 file:line，另给两条 🔵：① 设计档 §3.4 的内联样例与实现的完整消息不一致；② 类型守卫的严格性建议在 JSDoc 里写明。**两条均已处置**（`ea82924`：样例补全为逐字、JSDoc 补「平台契约保证 id 恒为非空字符串，非字符串一律视为缺失」；行为零改动）。
- **【会诊】**：`consult #6`（`zai-coding-cn:glm-5.3`）——本轮**不是普通的「第二意见」而是独立复算**：它自己 diff 了 0.1.6 备份树与桌面版 0.1.7-rc.2 树的五个包（把两处根因证据钉到行号）、实跑锁测试 **9/9**、并对 `compactMessages` 另跑 **1025 例对抗 + 1 组负控**。总判定：**两处根因均成立、目标形状逐字正确、同族消费点无漏改、批 24 无需返工**。父侧**逐条处置 7 条意见：采纳 6 · 不采纳 1**（唯一分歧 = 它把 `lib/consult.mjs` 的 `case "tool-result"` 判为死代码；父侧裁定**保留**——该分支是 0.1.6 时代创建、升级后续跑的会话的**跨版本读取兼容**，删掉会让这些会话的工具结果静默渲染成空串）。纪要：`docs/consult-minutes/2026-09-25-consult-6-minutes.md`（原始层 + 父侧裁定层 §2–§5）。**★ 计数纪律**：评审自报的 1025 例与负控脚本未随纪要交付 ⇒ 按「评审自报、父侧未复现」入册；父侧自己复跑的是 522/522 与两组变异。
- **【同批登记：一个平台缺陷（非本仓代码面）】**：`job_output` 读不回后台作业全文——2026-09-25 实测**两次复现**（`advisor-dsh-1` 与 `consult-1`），作业能派发、能跑完、完成通知也到，读全文时抛 `Cannot read properties of undefined (reading 'output')`。插件侧兜底已存在（评审正文进 `session-state.json`、会诊纪要落 `docs/consult-minutes/`）⇒ **结果不丢，但工具描述里「完成通知 + 全文经 job_output 读取」那句在当前平台上是死的**。已同步登记进 README「已知未决」。
- **【同批登记：下一批建议】**：会诊 #6 与设计档 §8 给出同一建议——**把插件的全部平台触点对着新旧两版 DSH 包做一次系统 diff**。它已帮本批摸过五类触点（`settings.yaml` 改名 / `subagents.start(spawn)` / `llm.stream` 的 GenerateOptions / `jobs` 的 `spec.run()` / agent-session 形状），其中 0.1.7 给 `spec.run(handle)` 新传的 `{ id, append, updateProgress }` 属**兼容性增量**（零参闭包自然忽略），进度上报登记为可选增强。
## [0.29.2] — 2026-09-21
