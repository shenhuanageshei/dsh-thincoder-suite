...
- **立项形态**：批 16 的形状谓词**能工作**，但**只扫两处**（档内声明的锚表与 AC 表）⇒ 六次逃逸**全部落在它的作用域之外**。**判据从「某张表内部自洽」升为「同一可数事实的每一处出现彼此同值」**：事实 id 是**唯一能让机器免猜地分组**的手段（id 即「同一性」的断言本身）。**★ 会诊原议的「自识别腿全档常开」已被父侧干跑实测否掉**（腿 3 常开 ⇒ 存量档红 34 处裸模块名）⇒ **本批增量只剩事实 id 腿**（`docs/2026-09-17-scope-alignment-design.md` §4 D17-0）。
