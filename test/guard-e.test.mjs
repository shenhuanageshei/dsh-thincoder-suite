...
//   A5/A6 = D-42a/D-42b · A7/A8 = 登记与授权面）。台账行由同一提交登记。
    ...
    // 批 29 单①（US-1）新增档（静默看门狗 + 工具面禁执行）——**在其落地的同一 stage 内登记**
    // （批 9「同 stage 登记」纪律：台账-历史一致闸的「T-E19 ↔ fs」等值腿会在「fs 已 +1 而本表
    // 未登记」时先红）。理由与设计档引用（docs/dsh017-batch29-design.md §2.1 / §5.1 / D29-2 ·
    // D29-6 / A29-1…A29-3 / AC-1…AC-3）：silence-watchdog.test.mjs = 静默看门狗的**注入缝**机验腿
    // （A29-3：到点 abort + 终止文案含「疑似挂死」与静默时长 · 有输出 ⇒ 续命不误杀）+ 工具面
    // 禁执行两态与**证据口径**（A29-1：强证 = 平台回显生效清单 / 弱证 = 派发载荷，两种形态在
    // 断言里区分；A29-2：取不到平台工具名清单 ⇒ 如实标注「未生效」，不假装拦住）。
