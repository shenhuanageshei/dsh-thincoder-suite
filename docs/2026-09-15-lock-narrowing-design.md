 ...
 *    **两个例外**（**修订**：初稿写「完全自由」过宽，审计给出反例）：
 *    ① **切片内不得新增 `/*`**（A2 域自检会红）；
 *    ② **不得改动右界标记的注释文字**（边界断言会红）。
 *    **例外**：切片内新增 `/*`（域自检会红）与**改右界标记的注释文字**（边界断言会红）。
...
// —— 夹具自检（D12-4）——
//   ★ 设计修订（§14.1 的 🟡#3）：初稿只有 `assert.equal(normalizeExec(FIXTURE), FIXTURE)`，
//     而审计**证明它不可达为首次失败**：`norm` 恒在 `normalizeExec` 的像集里 ⇒ `norm === FIX`
//     已经蕴含幂等 ⇒ 等值断言（更靠前）永远先红，幂等的消息**永不出现**（零边际检测力）。
//   ⇒ 改成**直接判定夹具是「已归一态」**，且**排在等值断言之前**（两条各自可独立致红）：
