...
| 2026-09-15 | **★★ 复核对账（子代理 `7bb31b24`）——推翻父侧一处、抓到一处硬冲突、补一处新锁面**：① **★ 父侧「三处锁」是错的，实为两处**——`test/advisor-config.test.mjs:145` 钉的是 **advisor 组 effort 经 `llm.stream` 的透传**（T3a，`:143` 的 `loopOpts({effort:"low"})`），**与 `engCoderEffort` 无关**；**勘察的两处（`:487` 默认值 · `:499` 非法值回落目标）是对的且完整**。★ **错法性质**：父侧做的是「`grep 'reasoningEffort, \"low\"'` ⇒ 3 处 ⇒ 当成三处锁」，**少做了「读上下文确认是不是同一把锁」** ⇒ **「字符串相同 ≠ 同一把锁」**（与本会话「模式匹配代替语境判断」三次同型）。② **★ 第二份悬空设计档**：`DESIGN-advisor-token-protocol-fix.md` 被 **3 处**引用（`lib/advisor.mjs:5` · `lib/advisor-msgs.mjs:5` · `lib/eng.mjs:6`），**盘上零命中、git 历史零命中** ⇒ **第⑥项的真实范围 = 10 处引用 / 2 份不存在的档**（勘察只报了 7 处 / 1 份）。**⚠ 而 `lib/advisor.mjs` 在批 15 的「明确排除（禁改）」清单里** ⇒ 若第二份入范围，**第⑥项会碰到批 15 冻结的档**。③ **★ 第⑤项有一处勘察未记录的锁**：`test/doc-hygiene.test.mjs:317` 把到点文案的**第二个字面**钉在 `SITES` needles 表里、**期望出现 2 次**（匹配 `lib/eng.mjs:1060` 与 `:1081`）⇒ **不能把两条重复的 `failStop` 文案合并成一条**（1≠2 ⇒ 红）；改**第一个**字面（`…超内部截止（budgetCapMs=` 那条）**安全**。**另** `:362` 钉 `failStop(` == 18（实测 18）⇒ **第⑤项新增的前置告警不得引入 `failStop(`**。④ **到点文案实为三处**（`:1017` console.warn · `:1059` failStop · `:1080` 与 `:1059` 逐字节相同），**勘察报两处**；且父侧 §7.8 的机理记述有误——**`Math.min(baseTimeout, budgetCap)` 是 codex 路径（`:875`），dsh 路径用的是裸 `budgetCap`（`:1020`）**（结论不变：键名归属误导）。⑤ **F8 精度订正**：**只有 128 分支**会打「第二句假警告」（它已先打过准确的 `:980`）；**ENOENT 分支总共只有一句 warn**。⑥ **行号已漂**：`README.md:169-172`→**`:191-194`** · `:227`→**`:249`**（成因 = 父侧 `be748c6` 在其上方插入 22 行）；`cordis.patch.yml:43` 正确。⑦ **谓词陷阱（实测）**：README `:194` 与 `cordis.patch.yml:15` 都出现 `engineering`，**朴素「提取所有标识符」会多抓它 ⇒ 误红**。 |
| 2026-09-15 | **★★ 硬冲突：同一行 `test/death-provenance.test.mjs:961`**——**批 14 要往 `AP_TEST_AUTHORIZED` 加 `test/advisor-config.test.mjs`**（第①项改默认值必须），**批 15 要往同一数组加 `test/consult.test.mjs`**（其设计档 `:410`）⇒ **同一行、两个批次各改一次**。**⇒ 必须串行**（批 15 先）。**另**：`lib/consult.mjs` 的文件头注释块**也是两批共同区域**（批 15 重写 `:2` 头注、批 14 改 `:5` 的 `DESIGN-dsh-port` 引用）⇒ **字面碰撞风险**。**而 `lib/index.mjs` 与 `README.md` 虽同档但区域不同**（`:177` vs `:1033`；`:249` vs 工具数小节）。**⚠ 另记**：批 15 设计档 `:415` 把 `test/advisor-config.test.mjs` 列入「明确排除（禁改）」——**那是批 15 的自限，不是全仓冻结**，批 14 第①项需要改它。 |
| 2026-09-15 | **★ 批 14 最终范围（6 项）**：**① `engCoderEffort` 默认值与回落目标**（`lib/eng.mjs:752` 与 `:759-760` 两次硬编码 `low`；README 与 patch.yml 示例都写 `low`；**三处锁见下**）· **② 白名单散文谓词**（`test/config-api.test.mjs`，权威面 21 名）· **③ 审计 F8 遗留**（`death-provenance` 的 not-a-repo 分支多打一句假 warn）· **④ V7 的 `topAllowed` 面** · **⑤ 同步/后台契约的代码侧文案**（`budgetCapMs` 之名被用在 dsh 路径上 · 加**前置告警**：同步路径 `budgetCap > ~600000` 时 spawn 前 warn）· **⑥ X-1 悬空引用**（**★ 复核对账后范围订正：10 处引用 / 2 份不存在的档**——`DESIGN-dsh-port.md` **7 处** + **`DESIGN-advisor-token-protocol-fix.md` 3 处**（`lib/advisor.mjs:5` · `lib/advisor-msgs.mjs:5` · `lib/eng.mjs:6`）；**⚠ 而 `lib/advisor.mjs` 在批 15 的禁改清单里** ⇒ **需裁定第二份是否入范围**，若入则本项要碰批 15 冻结的档）。**★ 实施顺序已定**：批 15 先行，本批排其后（见上一行）。**★ 另两处锁面须记入本批设计档**：**第①项的锁 = `test/advisor-config.test.mjs` 的 `:487` 与 `:499` 两处**（**不是三处**——`:145` 是 advisor 轴的断言，父侧曾误算）；**第⑤项的锁 = `test/doc-hygiene.test.mjs:317`**（到点文案第二字面**期望 2 次** ⇒ **不得合并两条重复文案**）+ **`:362` 的 `failStop(` == 18**（**新增前置告警不得引入 `failStop(`**）。**另**：本批**要往 `AP_TEST_AUTHORIZED` 加 `test/advisor-config.test.mjs`**——**与批 15 加 `test/consult.test.mjs` 是同一行 `test/death-provenance.test.mjs:961`** ⇒ **串行是硬要求，不是优化**。 |
| 2026-09-15 | **★ 第⑥项范围裁定（用户 2026-09-15：都纳入）**：范围 = **10 处引用 / 2 份不存在的档**——DESIGN-dsh-port.md **7 处**（lib/advisor-msgs.mjs:3 · lib/consult.mjs:5 · lib/eng.mjs:2 与 :232 · lib/state.mjs:2 · lib/readonly-tools.mjs:2 · lib/index.mjs:1136）+ **DESIGN-advisor-token-protocol-fix.md 3 处**（lib/advisor.mjs:5 · lib/advisor-msgs.mjs:5 · lib/eng.mjs:6）。**⚠ 其中 lib/advisor.mjs 在批 15 的「明确排除（禁改）」清单里** ⇒ **串行即解**（批 15 先实施完，本批再动它）。**修法二选一待会诊裁**：(a) 新建那两份档（**内容不可复原**——勘察已确证移植会话记录里只有引用、无正文）或 (b) **改引用落点**（指向真实存在的位置或改为自含描述）。 |
