...
| 2026-09-15 | **★ 范围缩减：批 14 由 6 项降为 5 项**（**批 15 会诊 R-1 的连带动作，用户已裁定 T-1**）：**原「A 项」整条删除**——那句「consult 没有完成通知、你必须自己回来」现已写进 `lib/index.mjs`（提交 `37d7eef`），而**批 15 要逐字反转它**（consult 改为走平台 `jobs` ⇒ 有通知）。⇒ **两批不能都做**（留着则批 15 后该描述为假；删则批 14 少一项）。**批 14 尚未开工 ⇒ 删它零成本**。⇒ **批 14 范围现为**：① `engCoderEffort` 默认值与回落 · ② 白名单散文谓词 · ③ 审计 F8 遗留 · ④ V7 的 `topAllowed` 面 · ⑤ 同步/后台契约的代码侧文案（**注意：这一项与批 15 的投递改造也要对齐先后**）。**`37d7eef` 的那段改动在批 15 里被反转**，历史不重写，留痕在批 15 的偏离表与 CHANGELOG。 |
