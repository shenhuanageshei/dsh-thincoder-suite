...
- 状态：**设计已通过评审（3 轮 + 专项自查 · `VERDICT: PASS`）· 已实施 · 携分歧审计 9 条待修复轮**（**★ 分歧审计 F11 订正：本行初写「设计待评审」，而 §12.1 已记三轮与 PASS**）
 ...
 *          会触发反向腿的**二次误报**（分歧审计 F10：现实现是 `continue`，需修正）
 */
...
| **`test/doc-hygiene.test.mjs`** | 追加**四条腿**（腿 4 含两道闸）· **`doc-shape` 声明解析** · **六条负控构造 / 五条顶层 test()** | eng_coder（凭 design token，**任务书指名该档**——D1 例外） | `node --test` |
...
| **A7** | 同上 | **每条腿的负控 + 反向腿的负控** | **四条腿（含腿 4 的两道闸）与反向腿各有 ≥1 条负控 = 6 条构造**（在**临时副本/临时档**上构造违规输入 ⇒ 必红；**不动被锁对象**——D5） |
| **A8** | 本批自己的两档 | `grep 'doc-shape'` | **两档都带声明**（**否则谓词对自己都不生效**——那是最讽刺的失败） |
| **A9** | 全仓 | 既有锁（台账 §三 · 六串 · `failStop(` · T-AP9 · R-25） | 全绿 |
| **A10** | 全量 | `node --test` | **用例数 = 基线 + 5**（**六条负控构造**，其中**五条各为一条新 `test()`**、反向腿并入第 4 条）且全绿——**★ 复审 #3 订正：本行原写「零新增用例数（相对基线）且全绿」，与 AC-10/AC-11 的「+5」互斥。而父侧上一轮只改了 §10.3 的 AC-10 行、本行未动，却在变更记录里宣称「A10 改为基线 + 5」——那是一句谎报，由本轮评审逐行读出**。**★ 本行是唯一未被任何 AC 引用的锚** ⇒ **按腿 4 的反向腿，它必须带显式豁免标记**，标记写法见 §5.4（`（豁免：全量锚，不挂单项 AC）`）。**★ 从本行起，本文档遵守自己定义的豁免语法。** |
...
>
> **★ 计数订正（分歧审计 F5）**：本批的**五道闸**（腿 1 · 腿 2 · 腿 3 · 腿 4 闸 A · 腿 4 闸 B）各有负控，**第 6 条覆盖反向腿** ⇒ **登记的负控构造 6 条 · 交付为 5 条顶层 `test()`**（反向腿并入第 4 条用例）。**两处口径已同值。**
>
...
| 文件 | 改动 | 类型 |
|---|---|---|
| `test/doc-hygiene.test.mjs` | **追加四条腿**（腿 4 含两道闸）——`parseDocShape` + 四个 check + **六条负控构造（5 条顶层 test()）** | **修改（核心，任务书指名——D1 例外）** |
...
> **★ 本表两行的订正史（从列表格中移出——分歧审计 F1）**：**T1 行原写「AC-11 · AC-12」**；复审 #4 判 AC-12 应为 T2。父侧当时的动作是**在 T1 行里写下一句「订正为 T2」而保留 AC-12 的字面**，只在 T2 行加了 AC-12 ⇒ **从「两行冲突」变成「一行自认冲突」**，而**机器读到的仍是字面 token ⇒ 判绿**。**⇒ 分歧审计 F1 的正确处置是：把订正语移出列表格**（上表两行现在**只含列表**），**因为括注里的字面就是让人与机器一起读错的原因**。**★ 附带教训：父侧的机械自查因「剥掉订正括注」而漏掉这一条——剥注是为了消除假警报，代价是同时屏蔽了「括注本身就是内容」的情形。** |
...
| **AC-10** | **存量 `docs/**` 零改动**（例外见 §8.1#3）；既有锁全绿；**★ 全量用例数 = 基线 + 5**（**六条负控构造**中**五条各为一条新 `test()`**、反向腿并入第 4 条）——**★ 评审 #3 订正：初版写「零新增用例数」，与 A7/AC-11 要求的负控腿直接矛盾** | T2 | A9 | US-5 |
| **AC-11** | **四条腿 + 反向腿各有负控**（**六条构造 / 共 5 条顶层 `test()`**：腿 1 · 腿 2 · 腿 3 · 腿 4 闸 A+B 各一条，**反向腿并入闸 B 那条**），且各**必须红** | T1 | A7 | US-6 |
...
| **F9** | 🔴 | **`shapeTable` 静默丢行**：AC 表中间出现一行非 `|` 开头 ⇒ **后续 AC 行整段消失且判绿**（实测：3 行只见 2 行，PASS）。**恒真入口 2.0** | **Dispatched**：**修复轮**（设计档需先把「**零行 / 行被吞**」写成一条闸） |
| **F10** | 🟡 | **闸 A 报红后该行被 `continue`** ⇒ 其 AC 号不进后续校验 ⇒ **真实存在的 AC 触发反向腿二次误报** | **Dispatched**：**修复轮**（§6 伪代码已写明「残缺行仍参与 AC 号提取」） |
