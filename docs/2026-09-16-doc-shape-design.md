...
> **★ 口径说明（代码评审第 2 轮 #4 补入）**：本节的图与阶段表里的「**五条负控腿**」是**初版计划口径**（当时只有腿 1/2/3/腿 4 闸 A/腿 4 闸 B 五条）——**与图 1 的「仍只画五道闸」同形**。**修复轮补入的第 6–8 条**（闸 0「行被吞」· F10「闸 A 后不得整行丢弃」· §9 零数据行闸）**不在本表内**，实交付为**九条构造 / 8 条顶层 `test()`**（见 §8.2 的自证腿表）。
