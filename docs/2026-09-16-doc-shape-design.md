...
| **`test/doc-hygiene.test.mjs`** | 追加**四条腿**（腿 4 含两道闸）· **`doc-shape` 声明解析** · **八条负控构造 / 8 条顶层 `test()`**（**★ 代码评审 #1 订正：本行初写「六条 / 5 条」，修复轮漏改本行与 §10.1**；**★ 代码评审 #6 订正：零数据行闸再补 1 条 ⇒ 九条 / 8 条**） | eng_coder（凭 design token，**任务书指名该档**——D1 例外） | `node --test` |
...
| **A7** | 同上 | **每条腿的负控 + 反向腿的负控 + 闸 0 的负控 + 零数据行闸的负控** | **四条腿（含腿 4 的两道闸 = 五道闸）+ 反向腿 + 闸 0 + 零数据行闸各有 ≥1 条负控 = 8 条构造 · 8 条顶层 `test()`**（在**临时副本/临时档/内存夹具**上构造违规输入 ⇒ 必红；**不动被锁对象**——D5）。**★ 2026-09-16 修复轮订正**：初版 6 条（五道闸 + 反向腿），**闸 0（行被吞，审计 F9）与 F10（闸 A 后不得整行丢弃）各补 1 条 ⇒ 8 条**（闸 0 独占负控腿 6 · F10 并入其后的负控腿 7）。**★ 代码评审 #6 订正：零数据行闸（§9 空集类②）补 1 条 ⇒ 8 条构造 · 8 条顶层 `test()`**（零数据行闸独占负控腿 8） |
| **A8** | 本批自己的两档 | `grep 'doc-shape'` | **两档都带声明**（**否则谓词对自己都不生效**——那是最讽刺的失败） |
| **A9** | 全仓 | 既有锁（台账 §三 · 六串 · `failStop(` · T-AP9 · R-25） | 全绿 |
| **A10** | 全量 | `node --test` | **用例数 = 基线 + 8**（**八条负控构造**，其中**八条各为一条新 `test()`**：五道闸 · 反向腿并入第 4 条 · **闸 0（负控腿 6）** · **F10（负控腿 7）** · **零数据行闸（负控腿 8）** ⇒ 实交付 **461 → 464**）且全绿——**★ 复审 #3 订正：本行原写「零新增用例数（相对基线）且全绿」，与 AC-10/AC-11 的「+5」互斥。而父侧上一轮只改了 §10.3 的 AC-10 行、本行未动，却在变更记录里宣称「A10 改为基线 + 5」——那是一句谎报，由本轮评审逐行读出**。**★ 本行是唯一未被任何 AC 引用的锚** ⇒ **按腿 4 的反向腿，它必须带显式豁免标记**，标记写法见 §5.4（`（豁免：全量锚，不挂单项 AC）`）。**★ 从本行起，本文档遵守自己定义的豁免语法。**（**★ 2026-09-16 修复轮第二次订正：+5 → +7**——两加两改同批完成：负控腿 6 与 7 各为一条新顶层 `test()`，台账 §三 `doc-hygiene.test.mjs` 单元格同步 **9 → 11**，`T-LC2` 等值锁实测通过。**★ 代码评审 #6 订正：+7 → +8**——零数据行闸再补一条新顶层 `test()`（负控腿 8），台账 §三 单元格同步 **11 → 12**，`T-LC2` 等值锁实测通过，全量实测 **464/464**） |
...
>
> **★ 计数订正（分歧审计 F5 + 2026-09-16 修复轮 + 代码评审 #6 修复轮 + 代码评审第 3 轮 #5）**：本批的**各道闸**（腿 1 · 腿 2 · 腿 3 · 腿 4 闸 A · 腿 4 闸 B · **腿 4 闸 0** · **腿 4 零数据行闸**）**逐一**各有负控，另加**反向腿**一条 ⇒ **登记的负控构造 8 条 · 交付为 8 条顶层 `test()`**（反向腿并入第 4 条用例 · F10 并入其后的负控腿 7 · **零数据行闸独占负控腿 8**）。**四处口径已同值**。**★ 图 1（§5.0）仍只画原设计的五道闸**——**闸 0 与零数据行闸都是修复轮补入的、不在图内**；**闸数换算以本行为准**。
>
> **★ 而「9 条」这个数是怎么来的（第 3 轮 #5 订正留痕，防它再回来）**：父侧曾把 **F10 那次代码修复**（`continue` 致整行丢弃）**当成第 9 条独立构造**，而**它其实是负控腿 7 里的一项断言**（负控腿 7 一次钉两件事：残缺行零丢失 + 闸 A 只报一条）⇒ **顶层 `test(` 的现实测值为 8 条负控腿（名字逐条可数），不是 9**。**⇒ 本档的判据：构造数 = 带「负控腿 N」的顶层用例数，它实测 = 8。**
>
...
| 文件 | 改动 | 类型 |
|---|---|---|
| `test/doc-hygiene.test.mjs` | **追加四条腿**（腿 4 含两道闸）——`parseDocShape` + 四个 check + **八条负控构造（8 条顶层 `test()`）**（**★ 代码评审 #1 订正：本行初写「六条 / 5 条」，修复轮漏改**；**★ 代码评审 #6 订正：八条 / 7 条 → 九条 / 8 条**） | **修改（核心，任务书指名——D1 例外）** |
...
| **AC-10** | **存量 `docs/**` 零改动**（例外见 §8.1#3）；既有锁全绿；**★ 全量用例数 = 基线 + 8**（**八条负控构造**中**八条各为一条新 `test()`**、反向腿并入第 4 条）——**★ 评审 #3 订正：初版写「零新增用例数」，与 A7/AC-11 要求的负控腿直接矛盾**；**★ 2026-09-16 修复轮再订正「+5 → +7」**（审计 F9/F10 各补一条负控腿 ⇒ 实测 **463/463**）；**★ 代码评审 #6 订正「+7 → +8」**（零数据行闸补**负控腿 8** ⇒ 实测 **464/464**） | T2 | A9 | US-5 |
| **AC-11** | **四条腿 + 反向腿 + 闸 0（F9）+ F10 + 零数据行闸（代码评审 #6）各有负控**（**八条构造 / 共 8 条顶层 `test()`**：腿 1 · 腿 2 · 腿 3 · 腿 4 闸 A+B 各一条，**反向腿并入闸 B 那条**，**闸 0 独占负控腿 6**，**F10 并入负控腿 7**，**零数据行闸独占负控腿 8**），且各**必须红** | T1 | A7 | US-6 |
...
> **★ 口径说明（代码评审第 2 轮 #4 补入）**：本节的图与阶段表里的「**五条负控腿**」是**初版计划口径**（当时只有腿 1/2/3/腿 4 闸 A/腿 4 闸 B 五条）——**与图 1 的「仍只画五道闸」同形**。**修复轮补入的第 6–8 条**（闸 0「行被吞」· F10「闸 A 后不得整行丢弃」· §9 零数据行闸）**不在本表内**，实交付为**八条构造 / 8 条顶层 `test()`**（见 §8.2 的自证腿表）。
...
| 轮 | 结论 | 处置 |
|---|---|---|
| **1** | **`VERDICT: FAIL`**（🔴1 · 🟡5 · 🔵1） | **7 条全 Fixed**——**🔴 是「修复轮宣称『改一处即改全部』而 §7 与 §10.1 被漏改、且无订正留痕」**；🔵 台账缺修复轮行。**评审正评**：「顶层 `test(` 实数 = 11 与三面同数」「**代码面未发现逻辑/资源/安全问题**」。**★ 唯一代码缺口**：`shapeTable` 的 `raw.length < 2` 对「表头 + 分隔线、零数据行」不充分 ⇒ **双侧空表时整体判绿**（单侧空被反向腿兜住）⇒ **派修复轮**（零数据行闸 + 负控腿 8）。 |
| **2** | **`VERDICT: FAIL`**（🔴1 · 🔵3） | **4 条全 Fixed**——**🔴 是同一个物种的第四次复发**：**#6 修复轮改了设计档五处 + 台账两处，唯独漏了 `CHANGELOG.md`（它前两轮都改过的登记面），且零留痕**。🔵：自证腿表第 9 行**被空引行从表中割离**（又是我，又在上一步刚加的行上）· 腿 3 的 `pathRe` 主干 `[\w-]+` **不含 `.`** ⇒ 多点文件名不进受检面 · §10.4 仍是初版「五条」计划口径。 |
| **3** | **`VERDICT: PASS`** | 上轮 4 项 **4/4 确认 Fixed**（四方口径实测同值：CHANGELOG / 设计档 / 台账 / 测试档头 · **顶层 `test(` = 12**）；**无新 🔴**。**余 1 条 🔵 已顺手结清**：**A7 行与计数订正行的枚举各列 8 项却断言「9 条构造」**——父侧曾把 **F10 那次代码修复当成第 9 条独立构造**，而它其实是负控腿 7 里的一项断言 ⇒ **已全档改为「八条构造」，并写明判据（构造数 = 带「负控腿 N」的顶层用例数，实测 = 8）**。**⇒ 这与「作用域不匹配」同源：谓词看不见 §8.2 的计数，所以 9 与 8 的差别只有人眼/独立审阅能抓。** |

> **★ 本批的账，如实记（这是本批最有价值的一条）**：**「任何『改了两处之一』都不算修好」这条规矩是本批自己立的（§11 的 §451），而它在父侧手里复发了四次** ——
>
> | 次 | 形态 | 发现者 |
> |---|---|---|
> | 1 | 批 14 设计评审轮 2 #1（只改了 AC-10 行，A10 锚行未动却宣称已改） | 设计评审 |
> | 2 | 分歧审计 F1（T1 行的订正语留在列表格里 ⇒ 机器读到的仍是字面 token） | 分歧审计 |
> | 3 | 分歧审计 F3 / 代码评审 #1（US-5 与 §7/§10.1 漏改） | 分歧审计 + 代码评审 |
> | 4 | **代码评审第 2 轮 #1（#6 修复轮漏改 `CHANGELOG.md`，零留痕）** | **代码评审第 2 轮** |
>
> **⇒ 四次全部由独立评审发现，零次由父侧自查发现**——**而本批交付的机器正是为了让这类缺陷被机器发现**。**⇒ 而它四次都没抓到，原因同一条**：**这四处都在「声明的锚表/AC 表」之外**（与 §12.2 末尾那条结论同源）。
>
> **⇒ 行动**：**「谓词的作用域与宣言对齐」已登记为候选批次**（让谓词也能看见 §7/§10.1/§11/CHANGELOG 这类登记面的计数与枚举）。
