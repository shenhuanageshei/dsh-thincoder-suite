...
- 状态：**已实施（v0.13.0）** —— 批 6 已交付（设计评审 → 实施 → 独立分歧审计 → 交付代码评审 `PASS`（🔴0 · 🟡1 经裁定并入 R-1 · 🔵4 已处置）；全量 `node --test` **360/360**）。**本行此前滞后写「设计待评审」，2026-09-13 批 10 文档卫生订正**
...
### §12.3 分歧审计修复轮落档（2026-09-13 —— **本追加与上文本冲突时以本追加为准**）

**背景**：修复轮交付 **360/360 全绿**（修前 356/355/1，唯一红 = D-37 锚，**已按父侧授权修成历史事实锚**）。8 条发现逐条处置并各带**可失败**证据（🔴 #1 给了「删生产站点即红」的实测输出：`'aborted'` vs `'aborted · abort(stop@agent: stop requested)'`）。本追加只记**设计面**回写与**登记**。

| 项 | 回写（**本追加为准**） |
|---|---|
| **§10.1 导出清单补全** | `lib/abort-provenance.mjs` 的导出实为 **10 个**（§6 点名的 7 个 + `abortTag`（D-AP6 机器短标签）+ `hostAbortInfo` / `hostAbortSource`）。**新增导出不是范围蔓延**——它们是 §12.2 #3 那条「未打标 ⇒ 宿主（`cancel@settle`）」判据的**唯一实现**，避免四家族各写一遍形状判定 |
| **§5.1 advisor 接线站 = 五处** | 已按 §12.2 #4 修正并落地（环内 `kind==="error"` 站接上死亡行；**普通 provider 错误逐字节零改动**，前缀 `Advisor: review failed — ` 保留） |
| **consult `cleanupConsultSessions` 面的可观测性（如实登记为例外）** | 该面**没有消费面**：宿主销毁路径会把会话从 `state.consultSessions` 摘除，之后 `consult_check` 只回 `unknown consult id`。故**只断言写点载荷**（`abort(cancel@settle)`）+「结算载荷确实按 `cancel@settle` 合成」（读会话自身队列，**非手搓等价物**），用例注释写明该限制。**仍保留该接线的理由**：abort 与载荷合成照常发生，未来任何终态回显消费者（§0.5 登记的墓碑最小形态 A-1）可直接读。**§5.1 的「接线站必须指定消费者」纪律在此记为例外并说明原因**——不得被读成「其余站也可以没有消费者」 |
| **stop 面则是真消费面** | 修复轮把死亡行 push 进该行自己的回复（`consult_check` 可见）——🔴 #1 的修法落点 |

**登记（未修，留后续批次裁定）**：

| # | 项 | 说明 |
|---|---|---|
| **R-1** | **codex job 的 `ABORTED` 信封路径仍无 `abort(...)` 标签** | 修复轮的 job-catch 站点**之外**的另一次元（`env.code === "ABORTED"` → `failureText`）；接它会改动既有断言串，超出本次 8 条范围 |
| **R-2** | **codex 面两站的测试构造有耦合点 + 一处测试侧副作用** | 依赖「`child.stdout` 取值发生在 adapter 的 `signal.aborted` 检查之后」这一实现事实；该处代码若变动，`T-AP1f/AP1g` 需同步调整（辅助函数注释已说明）。另：该构造会遗留一个 OS 临时目录（adapter 的 cleanup 只在正常路径跑）——**测试侧副作用，不影响产品语义** |
| **R-3** | **T-AP9 锚 A 在提交前处于「未激活」态** | 打印 `console.warn` 而非静默假装通过；提交批 6 后自动生效 |
