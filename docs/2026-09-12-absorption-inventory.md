...
|---|---|---|---|---|---|---|
| ENGINEERING-MODE | 工程模式定性 + 批次档六段 + batchDoc 门禁 + 失败路径 11 条 | PARTIAL：流程已备；无批次档体系 | 载体 T-ONLY；**失败路径 11 条 PORTABLE** | 批 11（只取语义） | —（已闭环） | 中 |
| ENG-DESIGNER | eng-designer 角色 + 文档更新纪律 D1–D7 + V1/V2 校验 | MISSING（角色）/ D1–D7 未成文 | NEEDS-ADAPT | 批 11（只取 D1–D7） | —（已闭环） | 中 |
...
| DOC-HYGIENE | 文档卫生 6 条 | PARTIAL（无机械检查） | NEEDS-ADAPT（V1 段引用可解析起步） | 批 11 | —（已闭环） | 中 |
...
| **批 11** | **D1–D7 文档纪律**（ENG-DESIGNER 的文档更新纪律）+ 上游 **ENGINEERING-MODE 的失败路径语义**（只取语义，载体 T-ONLY）+ **DOC-HYGIENE 的提示词级纪律**（机械检查器**不吸收**，见 §5-6） | 依据 | §2 建议批次列的**批 11 列**（`批 11` 摸底订正：初写「4 条命中」，**实测恰 3 条**——`ENGINEERING-MODE:39` / `ENG-DESIGNER:40` / `DOC-HYGIENE:57`；见批 10 行的变更理由③） | 仅供参考（§2 的批 11 行 = 本行的域，逐条已对账）。**2026-09-13 批 11 stage 7 变更理由行（按 §7 规则记）**：① §2:84 的计数「共 **4** 条」→ **3 条**（那个 4 是 POOL-LEDGER 改挂批 10 **之前**的旧真值；计数与列表现同一次编辑改毕，依据 = §2 表体 42 行中「建议批次」列匹配 `批 11` 者**恰 3 行** `:39`/`:40`/`:57`）；② 该三行的**触发列同步标 `—（已闭环）`**（仍命中 J10-3 的闭合四值，由 `test/ledger-parity.test.mjs` 的 T-LP6 机检）；③ 本行内容 = **实际交付三件**：D1–D7 成文（规范层进 `METHODOLOGY.md` 新子节、**现状标定只进设计档 §10**）· 上游失败路径语义（只取「停产报告 + 不自动重派」，落 `lib/eng.mjs` 返回串与 `lib/index.mjs` 的 eng_coder 描述**同批划界**）· DOC-HYGIENE 提示词级纪律（`lib/prompts/discipline.md` 五条 + 新档 `test/doc-hygiene.test.mjs` 两项机械检查）；一般性文档检查器**照旧不吸收**（§5-6） |
| **不立项** | §5 全部 | 宿主不同构或无实测痛 | — |
