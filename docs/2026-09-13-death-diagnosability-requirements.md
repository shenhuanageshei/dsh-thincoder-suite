...
## §8 设计评审轮次 1 修正块（2026-09-13 —— **本追加与上文本冲突时以本追加为准**）

**背景**：设计评审轮次 1 = `VERDICT: FAIL`（**🔴1 · 🟡6 · 🔵4 = 11 条**，job `advisor-dsh-2`；计数已按评审表实际值订正——评审轮次 2 的 🔵 #13）。本档相关的两条：

| 评审 # | 级别 | 处置（**本追加为准**） |
|---|---|---|
| **#2** | 🟡 | **US-1 的「四分辨」构造集改为 `{user, timeout, cancel, unknown}`**——原文写「四家族各构造 user / timeout / cancel / **crash** 四种死亡」，但 `TRIGGERS` 五值里**没有 crash**：真 crash 是**非 abort 错误**，按 D-AP5 **不带** abort 后缀。故 **crash 单列为第五个构造**，其判定句 = 「**无** abort 后缀 ∧ 原 message 逐字」。US-1 的判定句按此读：四触发分辨 + crash 透传 |
| **#6** | 🟡 | **US-4 的「看得见」口径订正为「评审员看得见」**——提示的通道是 `messages` 注入（**模型面**），不是用户界面；原文「作为跑长预算评审的人，我希望预算将尽时看得见一次提示」会把 AC-AP4 引向一个不存在的 UI 断言。**订正后的故事**：「作为跑长预算评审的人，我希望**评审员**在预算跨过 75% 时**被提示一次**（从而主动收尾），且该提示**不污染**返回正文与 prior」。AC-AP4 只断言**注入面**（一次性 + 不进正文/prior）——与真实通道一致 |

**未变**：§1–§7、A-1…A-4、其余用户故事与非功能标准。**设计侧的对应修正见设计档 §12**。
