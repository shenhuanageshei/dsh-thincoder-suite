...
| **12** | ★ **`test/codex-runner.test.mjs` 的 5 行既有断言被翻转**（**审计 🟡-3 补登**） | 该 5 行属「R2 D-06 复合键 single-flight」用例：**FR-2 的护栏要求 code 型评审在 eng/escalate 在飞时被拒** ⇒ 旧的「异机制派发不受阻碍」断言**必然与之矛盾** ⇒ **翻转是被迫且正确的**（N5 实测：拆掉护栏 ⇒ AC-7 两腿转红，证明新行为被锁住）。**⇒ 这是记录面缺口，不是行为缺陷**；已同批登记（台账 §七 与本表） |
...
| **N9** | **畸形输入 fail-safe**（★ 评审 #5 补 · **★ 审计订正**） | **★ 审计实测：原写法的字面形态（`typeof v === "boolean" && v === false`）在语义上与现实现全等 ⇒ 恒绿、不可能红（R2 衰变锁物种）** ⇒ **改用意图形态**：判定改成**只接受 `true`/`undefined` 为后台**（`args?.background === undefined \|\| args?.background === true`）⇒ 字符串等非布尔值被误判为「同步」 | **AC-15 / A14**（**审计实测：67/66/1，恰红 AC-15，零误伤**） |
...
| **M4** | **codex 路径与 background 相关的**既有断言全绿（**★ 审计订正**：字面「既有断言全绿」不成立——**FR-2 的护栏迫使 5 行既有断言翻预期**，见 §8.1 新增第 12 项与台账 §七） | 零漂移（除那 5 行的被迫翻转） |
...
| **8** | ★ **`lib/eng.mjs` 的前置告警尾句仍写「长任务请传 `background=true`」**（**交付代码评审 🔵#2 登记**） | 翻转后「省略即后台」⇒ 该指引**功能上仍正确但冗余**；它**不在 FR-5 的权威六处清单内**，且尾串可能被 `doc-hygiene` / `advisor-config` 的既有断言钉住 ⇒ **本批不动**（改它要先核锁），登记为后续候选 |
| **7** | ★ **AC-9 / AC-10 的机器锁只覆盖一部分**（**审计 🔵-5 登记**） | AC-9 的过渡态/隔离期纪律**无任何断言**（只有实读证据）；AC-10 的六处描述面**只有 2 处**（两处 schema 描述）被 A3 锁定，`:950`、README 那节、两处头注、token-lifecycle 那 4 处**无断言** ⇒ **后续静默编辑不会被抓**。**这是 §10.2 已声明的 T2 取舍，不算违约**；后续批次可并入 doc-hygiene 正向锚 |
...
**交付方式**：实施跨 **4 次派发**——其中**两次以 `error` 零产出结束**（工作树干净、无残留），**根因是 `ctx.jobs` 服务未挂载**（profile 缺 `@deepseek-ai/dsh-jobs-local`）⇒ 首次「探测」证实后台派发已恢复（不传 `background` 也返回 job 句柄）。

| 核验项 | 实测（从盘上量，不采信报告） |
|---|---|
| 全量 `node --test` | **509 / 509 · 0 fail**（基线 497 + 本批 12） |
| 顶层 `test(` 逐档合计 | **509 == 台账 §三 合计 509**（**T-LC2** 一致） |
| 禁改面 8 项 | **`--numstat` 全空** |
| 授权数组 | `AP_TEST_AUTHORIZED` **行逐字未变**（`death-provenance` 只 +14 行注释） |
| `failStop(` / 六串 / 零新依赖 / 零新键 | **18** ✔ · `lib/**` 30 档各 **0** 命中 ✔ · `package.json` 空 ✔ · `topAllowed` 无 diff ✔ |
| **新腿在旧实现下必红** | 审计实测：`e131c10^` + 新腿 ⇒ **509 / pass 499 / fail 10**，**全部 `ERR_ASSERTION`、假红 0** |

### §12.3 交付代码评审落档（**`advisor type='code'` · job `advisor-dsh-1` · 轮 1 = `VERDICT: PASS`**）

**结论**：**`VERDICT: PASS`（🟡1 · 🔵3）**——**四处发现全部是注释/描述层与历史快照的滞后，零行为缺陷**（评审原话：「未发现逻辑错误、竞态、资源泄漏或静默失败路径」）。

**评审正评（照录）**：三态判定 `eng.mjs:949` / `escalate.mjs:437` · schema 描述与 `default: false` 已清 · **透传 `index.mjs:1013` 裸传** · **codex 三处仍 `=== true`** · 护栏住 `advisor.mjs:1922-1927` 的**零副作用位置**（先于 F11 链重置）· 回落**双通道 + 文案逐字节**（abort 家族的例外有双侧锁依据注释）· 提示词两段实际在位 · 六处描述面逐一在位 · 授权注释第⑥条 + **数组行逐字未动** · 交接页重启行与四条可感变化 · AC-14/AC-15 齐备；**「台账、交接页、设计档三处计数（509/67/190/12）互证一致」**。

| # | Action | Detail |
|---|---|---|
| **1** | **Fixed** | `test/advisor-config.test.mjs:534` 的注释「默认 background=false」在翻转后失真 ⇒ **改为如实措辞**（并写明批 21 起同步路径只剩两种到达方式）；纯注释、授权面内零仪式 |
| **2** | **Deferred** | `lib/eng.mjs:1060-1062` 的前置告警尾句仍写「传 `background=true`」⇒ **冗余但正确**，且**不在 FR-5 权威六处内**、尾串可能被既有断言钉住 ⇒ **登记为 §9.3 第 8 项**（后续候选） |
| **3** | **Not an issue** | `lib/escalate.mjs:567-569` 的三元式「`ctx.subagents` 缺失」臂**结构上不可达**（R5 遗留的死分支）⇒ **评审自己也判「本批不动是正确的」**：本批对该区域有**文案逐字节的硬约束**（D-32-5），动它等于破坏该约束 |
| **4** | **Fixed** | `docs/2026-09-13-handoff.md:155` 的 §6「第一小时建议」仍是**批 11 时点快照**（写基线 453）而当前为 509 ⇒ **补 as-of 括注**并指向 §1 批次表 |

**⇒ 2 `Fixed` · 1 `Deferred`（已登记）· 1 `Not an issue`（附评审自身的判断）；无 🔴、无 `Dispatched`。**

### §12.4 分歧审计落档（**只读子代理 `f968f39f`**）

**结论：分歧 5 条（🔴1 · 🟡2 · 🔵2）· 阻塞项 0。** 仓内零残留 ✓。

| # | 级 | 发现 | 父侧处置 |
|---|---|---|---|
| **1** | **🔴** | **AC-13 / A15 零证据**：交接页无批 21 重启行（`git log -1 -- docs/2026-09-13-handoff.md` = 批 20 的 `d76adc1`） | **Fixed**（本次收口补写：§1 批次行 + 重启清单条目） |
| **2** | 🟡 | **N9 的字面形态不可能转红**（与原实现语义全等 ⇒ 恒绿）；意图形态（只接受 `true`/`undefined`）才恰红 AC-15 | **Fixed**（本档 N9 的构造已按审计改写）；**代码无需改** |
| **3** | 🟡 | **5 行既有断言被翻转，三处记录都没写**（本档 M4 的「既有断言全绿」字面不成立 · 台账 §七 未提翻预期 · 授权注释写「各新增」） | **Fixed**（本档 M4 订正 + §8.1 新增第 12 项 + 台账 §七 补记）；**代码无需改** |
| **4** | 🔵 | §12.1 指向的 §12.2/§12.3/§12.4 当时不存在 | **Fixed**（本次收口落档，本页即其兑现） |
| **5** | 🔵 | **AC-9 / AC-10 的机器锁只覆盖一部分** | **已登记**（§9.3 新增第 7 项；属 §10.2 已声明的 T2 取舍） |

**★ 审计的正面实证（照录，供后续会话采信）**：**12 条新腿在旧实现下恰 10 条红（全部 `ERR_ASSERTION`，零假红；另 2 条是逃生口回归锁）** · **10 条负控 N1…N9b + 5 条阴性对照 M1…M5 逐条实跑、零假红**（**N2** 把 `index.mjs` 透传改回 `=== true` ⇒ **AC-2 转红，三态透传的必需性得证** · **M1** 见证「回落同步」硬约束：两档用 `jobs: null` 的 deps，若回落改 fail-closed/抛错必红）· **回落文案用 `git show` 逐档比对 ⇒ 集合逐字节相等** · **台账 §三 21 行 == fs 21 档，双射、逐档 21/21 全等、合计 509 == 509**。

**★ 审计给出的「我无法验证的」6 项**（如实转登记）：**重启后是否已加载新 `lib/**`**（T3 人工面）· 实施者自称的负控产出物**无盘上物可查**（本档 §12.2/§12.4 由其复跑替代）· **那两次零产出失败当时的真实病因**（挂载时序不在其可观测面）· **N1/N3/N7 的额外红是否算分歧取决于「下界 vs 唯独」口径**（审计按**下界**判 ⇒ 不算）· **§8.1 零改面清单本身的封闭性**（无法证明没有漏项）· 设计档自己登记的不可本仓内生验证项（真机长任务 · 重启失忆发生率 · 别的交错组合 · 提示词效果）。
