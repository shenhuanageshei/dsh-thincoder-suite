...
- 状态：**设计档（事后回填）**——本批走的是「先修后补设计档」的应急路径（决策 **D24-5**），故本档在实施完成后回填；**§7 是实施后实测记录，不是设计期预期**。口径分离，避免把事后数值读成设计承诺。若本档早于实施存在，本节应写「设计待评审」。**★ 改 `lib/**` ⇒ 重启 DSH 后生效**
...
| 项 | 结果 |
|---|---|
| 全量 `node --test` | **522/522 绿**（基线 516 + 本批 6：D-40 的 `config-api` +1 · 新档 `dsh017-compat.test.mjs` +4 · `truncation.test.mjs` +1（**用例数 4 → 5**，与 A7 / AC-8 / 台账 §三 / fs 实测四方同值）） |
