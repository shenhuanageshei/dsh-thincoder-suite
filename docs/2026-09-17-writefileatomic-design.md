...
| # | Action | Detail |
|---|---|---|
| **1** | **Fixed** | **AC-11/N-14 的疗效门槛是收口硬前置**（它认可台账「待收口前跑并记录」是**诚实不造假**，但要求跑完再升措辞）⇒ **父侧已跑**（全量 ×10 + `session-state`/`codex-runner` ×20），**读数落 §12.5**，随后把 §五 的「暂记」升级为终态 |
...
**结果**：**见下表（父侧实测，job `pwsh-43`）** —— 达标后，台账 §五 的「机制已根修；疗效门槛待收口前跑并记录」**升级为终态「已根治（`v0.28.0`）」**。
