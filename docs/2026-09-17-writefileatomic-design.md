...
| **5** | ★ **预算与墙钟的两个口径（审计实测，父侧订正 N-3 的措辞）** | **审计用真 fs（目标建成目录）实测**：缺省路径 ⇒ `code=EPERM` `errno=-4048` `attempts=8` **`elapsedMs=311~312ms`**。⇒ **本档与需求档 N-3 写的「最坏 ~210ms」是「睡眠预算」（7 × 30ms），真实墙钟 ≈ 1.5×**。**订正后的口径**：**睡眠预算 210ms / 真实墙钟约 310ms（同机实测）**，两者都在「同步阻塞预算须小」的可接受范围内 |
...
| **A2** | 同上 | `renameSyncWithRetry` | 定义**恰 1 处**且**被两个消费者调用**（`writeFileAtomic` + `clearUserConfig`）。**★ 审计订正（2026-09-17）：本条是「结构性锚」——「被两个消费者调用」纯静态、无机械腿**（把 `clearUserConfig` 的调用换成正确 import 的裸 `renameSync` ⇒ 全套件 **0 红**）⇒ **降为静态/结构性锚**，并**已派修复轮补源码级锁** |
...
| **A9** | 同上 | **首写失败清理** | 注入写失败 ⇒ **零 tmp 残留** + 抛。**★ 审计订正（2026-09-17）：本条是「结构性锚」，不是「改前红/改后绿」**——实测 `writeFileAtomic(target, {})` 在 **新旧两版都零残留**（两版皆 `ERR_INVALID_ARG_TYPE` 且 `writeFileSync` **在建文件之前就抛**），Node v24.14.1 上试了 4 个候选触发**无一**能让它「先建文件再抛」；**唯一为真的部分**是**静态事实**：`35cd744^:lib/dsh-home.mjs:69` 的写确在 `:70` 的 `try` **之外** ⇒ 该锚的判据 = **源码结构**（写是否在环内 `try`），**不是**动态红绿 |
...
| **N4** | 首写失败清理 | 把写移回环外（并去掉 `write` 钩子 = 旧码真实形状） | **★ 审计订正（见 §8.3 尾注）：实测红在 A7 + A8，A9 仍绿** ⇒ 原期望「红在 A9」**不成立**，已改按实测登记 |
| **N5** | 遥测 | 把 message 的追加删掉 | **A4**（签名/次数缺失） |
| **N6** | 孤儿清扫阈值 | 把 10min 改成 0 | **A10 的阴性对照**（新 tmp 被误杀） |
| **N7** | TOCTOU 幂等 | 把 `ENOENT ⇒ true` 删掉 | **★ 审计订正：实测零红（9/9 绿）** ⇒ 原期望「红在 A11」**不成立**（A11 走的是前置早返回、从不进该 catch 分支）⇒ **缺口已派修复轮补断言** |
...
> **★★ 审计尾注（2026-09-17，`5c987f01`）——一条**方法论**级的订正，本档与后续批次都适用**：
> **「每条新断言都在旧实现下必红」这条对本批的「**新档**」不成立。** 实测：把 `lib/**` 退回 `35cd744^`（**byte-exact**，`git cat-file -s` 4556 / 22964）但保留新的 `test/write-atomic.test.mjs` ⇒ **旧 revision 不导出 `RENAME_BACKOFF_MS`** ⇒ 新档在 **import 阶段整档 `SyntaxError`** ⇒ **pass 0 / fail 1，零条具名用例红** —— 而这**正是本档 §8.3 自己定义的「假红」**（`ReferenceError` / 整档崩溃 = 假红）。
> **⇒ 对新档，可用的反恒真判据不是「退回旧实现」，而是「逐条变异」**（审计已跑：**N1 ⇒ A1+A6+A6b · N2 ⇒ A7+A9 · N3 ⇒ A8（正好） · N6 ⇒ A10（正好）**，**四条全为断言级真红、零整档崩溃**）。
> **⇒ 已如实登记的两条不符**：**N4 实测红在 A7+A8（非 A9）** · **N7 实测零红** ⇒ **§8.3 表里那两条期望值原先是「写着但没人能执行」的物种**（与本仓批 11 记下的元缺陷同源），**今按实测值改写**，缺口另派修复轮补断言。
> **⇒ 审计的自纠（口径）**：它起初判「CR 幻影不复现」，**随后用正确口径订正**——根因是它用了 **commit↔commit** 比对；**并给出本机的一条重要提醒**：**worktree 与 blob 的 EOL 不一致是全机既有状态**（连批 20 从未触碰的 `lib/index.mjs` 也如此：blob 76738 / worktree 78010 bytes），**被 git 的 stat-cache 掩盖**（故 `porcelain` 空）⇒ **零改面判定应当用 `git diff --numstat <旧rev> <新rev>`（commit↔commit），不要只用裸 `git diff`（对 worktree）**。
...
| **11** | **测试档内那处「改前必留半个 tmp」注释未同改**（评审 #3 的第三处） | `test/**` 属实施域、且**仅注释级** ⇒ 不在父侧小修面内，**登记为残差**（两处**文档**已同批改为「结构判据」；本行即其缺口记录） |
| **12** | **A1 锚的白名单腿只有负向覆盖**（评审 #5） | 删掉 `RENAME_RETRYABLE_CODES` 里的 `EACCES`/`EBUSY` ⇒ **全套件零红**（正向重试腿只测了 `EPERM`）。**登记**：白名单是**带注释的常量** ⇒ 改它是一次**显式动作**（与 R-61 同款逻辑）；补**参数化正向腿**列为后续候选 |
| **13** | **M1/A8 的墙钟断言有理论假红面**（评审 #7） | 用 `Date.now()-t0 < 500` 判「无环内 sleep」——病态慢 fs 上会假红。**A8 已有确定性的 `calls === 1` 腿兜底**；**M1 无计数腿** ⇒ 补腿列为后续候选 |
| **14** | ★ **疗效门槛未达标 + 一条身份未知的偶发**（**本批最重的一处未决**） | 全量 ×10 里 **1 红**（`fail=1`，**哪一条未知**：父侧脚本只记计数，**未捕获失败项名**——**测量设计失误**；追加 20 轮 **0 复现**，累计 30 轮 1 红）⇒ ① **`R-13` 不能宣告「已根治」**，台账 §五 保持诚实措辞；② **那 1 红与 `R-13` 的历史签名同形但未命名** ⇒ **既不能排除也不能断定**；③ **后续候选**：把「全量 ×10 的失败项名」纳入任何复跑协议（**父侧脚本的教训**），并在下一次全量复现时定位 |
...
| **AC-4** | **首写失败** ⇒ **零 tmp 残留** + 抛（**★ 评审 #3 订正：不写「改前必留半个 tmp」**——那是**动态不可证**的；**判据 = 源码结构**：写步骤在环内 `try` ⇒ 失败即 `unlink`。见 §8.2 A9 的审计订正） | T1 | A3 · A9 | US-1 |
...
| 2026-09-17 | **★ 交付与收口落档**（`v0.28.0` / `33b1bdc`）：§12.2 补**第 4 次派发**与 §12.3 交付代码评审（`VERDICT: PASS`，7 条）；**新增 §12.4 疗效门槛**（**未达标**：全量 ×10 = 9/10，一条身份未知；追加 50 轮 0 红 ⇒ 累计 1/80；★ **不因事后绿块而追溯满足门槛**）与 **§12.5 分歧审计**（10 条，含 1 🔴）。**★ 父侧事故留痕（本会话第五次同形）**：**写 §12.4/§12.5 时，父侧把新小节插在了对 §12.3 的编辑里 ⇒ 造成「§12.4 排在 §12.5 之后」的乱序**——**由「数小节编号」这个机械动作发现**（不是靠读全文）。已按「让文件顺序与编号单调」修：**疗效门槛 = §12.4 · 分歧审计 = §12.5**，并同步 §12.1 的指针与台账的引用。 |
...
- **交付核验 / 分歧审计 / 交付代码评审**：见 **§12.2 / §12.5 / §12.3**；**疗效门槛落档**见 **§12.4**。
...
**★ 第 4 次派发（`eng-dsh-14`，收窄到「只做测试档 + T-E19」）：成功**——`test/write-atomic.test.mjs` **246 行 / 9 条用例 / 9-9 通过**（**超出父侧设的最低门槛**：覆盖 A1 · M1 · A6 · A6b · A7 · A8 · A9 · A10 · A11）· `test/guard-e.test.mjs` +8/-0（T-E19 20→21）· 测试档 **20 → 21**。
**★ 台账由父侧补**（D1 登记面分工）：§三 新增 `write-atomic.test.mjs` 行（9）· §五 的 `R-13` 行**保留**并标注已根治 · §七 批 20 历史行 ⇒ 全量由 **491/3 红** 转为 **494/494**（红的三条正是 `T-LC1`/`T-LC2`/`T-LC4` 的四方一致闸）。

### §12.3 交付代码评审落档（**`advisor type='code'` · job `advisor-dsh-4` · 轮 1 = `VERDICT: PASS`**）

**送审面**：`lib/**` 五档 + `test/write-atomic.test.mjs` · `test/config-api.test.mjs` · `test/guard-e.test.mjs` · `docs/test-lifecycle.md`；文档面 = 本档 + 需求档 + 摸底档 + 纪要。

**结论**：**`VERDICT: PASS`（🔴0 · 🟡3 · 🔵4）**。**正评**：「**全部声称的交付物均在位且与设计一致**；注入缝族（`{delays, maxAttempts, write, onEnoent, rename}`）设计干净——**`write` 钩子的存在性门控 `unlink(from)` 精确保护了 `clearUserConfig` 的活文件**；`A11b` 用 `Date.now` 缝确定性构造 TOCTOU 窗口并带**前置自证**，`A2` 源码级锁带 **EOL 归一与恒真空切片防护**，两条修复轮腿的负控口径与会话自报吻合；**遥测格式与 §5.1b#3 逐字一致**；**三个 store 契约零变更**；零改面与登记级联完整」。

| # | Action | Detail |
|---|---|---|
| **1** | **Fixed**（**结论「未达标」，已如实落档**） | **AC-11/N-14 的疗效门槛**：**已跑**（全量 ×10 + `session-state`/`codex-runner` ×20）⇒ **全量 9/10（一轮 `fail=1`，身份未知）· 单档 20/20** ⇒ **门槛未满足** ⇒ **台账 §五 保持诚实措辞、不升级为「已根治」终态**；读数与三条如实登记见 **§12.4** 与 **§9.4 残差 #14** |
| **2** | **Fixed** | **AC-13（交接页重启行）**——**收口时由主代理写入** `docs/2026-09-13-handoff.md` |
| **3** | **Fixed** | 「**改前必留半个 tmp**」的旧口径未随审计订正对齐（**三处**：本档 AC-4 括注 · 台账 §三 该行 · 测试 `A9` 注释）⇒ **两处文档已改为「结构判据」**；**测试档内那处注释**（`test/**` 属实施域）**登记为残差**（见 §9.4 #11） |
| **4** | **Fixed** | **D3 计数-枚举不同步**：台账 §三「锚覆盖」枚举仍停在 9 条（`A11b`/`A2` 未列）· §七 仍记「9」 ⇒ **两处已同批改**（枚举补 `A11b` · `A2`；§七 补修复轮记录，**11**） |
| **5** | **Deferred** | **A1 锚的白名单腿只有负向覆盖**（删 `EACCES`/`EBUSY` ⇒ 零红）⇒ **登记为 §9.4 #12**（白名单是**带注释的常量**，改它是一次显式动作——与 R-61 同款逻辑；补**参数化正向腿**列为后续候选） |
| **6** | **Fixed** | **`clearUserConfig` 的 `!filePath` 分支静默 `return false`**，而 DELETE 的 500 文案写「see console warn」 ⇒ **父侧评审后小修（minor fix）**：补一行与 `saveUserConfig` 同形的 `warn`（**行为零变更**，`lib/config-store.mjs:224-229`）；`config-api` **26/26** 复验通过 |
| **7** | **Deferred** | **M1/A8 的墙钟断言理论假红面**（`< 500ms`）⇒ **登记为 §9.4 #13**（A8 已有确定性的 `calls === 1` 腿兜底；M1 补计数腿列为后续候选） |

**⇒ 无 🔴；4 条 `Fixed` · 2 条 `Deferred`（均 🔵 且已进 §9.4 残差）；0 条 `Dispatched`。**

### §12.4 疗效门槛落档（**AC-11 / N-14 / 锚 A16**）

**协议**：全量 `node --test` **×10** + 单档（`test/session-state.test.mjs` + `test/codex-runner.test.mjs`）**×20**，**要求全绿**（`fail 0` 且 `pass > 0`）。

**结果（父侧实测，job `pwsh-43`；as-of 2026-09-17）**：

| 跑法 | 全绿 | 红 |
|---|---|---|
| 全量 ×10 | **9/10** | **全量#9：`fail=1`（`pass=496`）** |
| 单档（`session-state` + `codex-runner`）×20 | **20/20** | 无 |

**⇒ ★ 门槛判定：未达标（N-14 的「全绿」未被满足）** ⇒ **台账 §五 的措辞保持诚实形态**（「**机制已根修（v0.28.0）；疗效门槛待收口前跑并记录**」），**不升级为「已根治」的终态结论**。

**★ 追加 50 轮（**带失败项名捕获**，job `pwsh-44`；每轮 ~16s）：`0` 红 ⇒ 累计 1 红 / 80 轮 ≈ 1.25%。**

**★ 门槛判定不变（防「跑到绿为止」的取样作弊）**：预注册的判据是「**一次 ×10 全绿**」，而**首跑即失败** ⇒ **事后重跑出绿块不能追溯地满足它**。⇒ **台账措辞保持「疗效门槛未达标」**，不升级为「已根治」。

**★ 本次追加的实质收获（两条，都是**排除**与**效力反省**，不是结论）**：

| # | 内容 | 读数 / 判据 |
|---|---|---|
| **1** | **弱排除一个嫌疑面**：本批**新加的用例里有墙钟断言**（评审 #7 点过 `M1`/`A8` 的 `< 500ms`） | spec reporter 实测 **M1 = 5.66ms · A8 = 6.64ms** ⇒ 余量 **88× / 75×** ⇒ 要它们假红需 **~80–100× 的 fs 退化**，**不合常理** ⇒ **该嫌疑面弱排除**（**不是确证**；且这仍是「结构上可能假红」的物种，已登记为 §9.4 #13） |
| **2** | **★ 反省：单档 ×20 这个协议对 `R-13` 的检出效力很低** | ① 我用的是 `session-state` **+** `codex-runner` **两档一起**，而台账里 `R-13` 的历史证据是「**单独**复跑 10/10 绿」⇒ **口径不同**；② 以观测到的 p ≈ 1.25% 计，**20 轮全绿有约 78% 的概率自然发生** ⇒ **20/20 几乎不能排除任何东西** |

**⇒ 未决项的准确表述（收口定稿）**：**「80 轮全量里 1 红、失败项身份未知；既不能排除它是 `R-13`，也不能认定它是 `R-13`。」** 已排除的只有一项（新档墙钟断言的余量，弱排除）。**后续候选**：在任何复跑协议里并入**失败项名捕获**（本轮已做）· 在**机器安静**时跑更长块 · 若下次复现的是 `session-state.test.mjs :: T6` 则须在 `writeFileAtomic` 之外找第二因。

| # | 事实 | 读数 |
|---|---|---|
| **1** | **失败身份未知** | ×10 的脚本只记了计数（`fail=1`）**没有记失败项名字**——**这是一次测量设计失误**（父侧自己写的脚本：应当捕获 `not ok` 标题）。**追加 20 轮未能复现** ⇒ **只能登记为「身份未知的一次偶发」** |
| **2** | **「全量偶发红 + 单档全绿」与 `R-13` 的历史签名同形** | ⇒ **不能排除它就是 `R-13`**，也**不能断定**（它也可能属于另一个既有 flake 面） |
| **3** | ★ **而「不删 `R-13` 那行」这个决定因此被二次印证** | 台账 §五 的行**仍在** ⇒ 发布门 `G6` 的**复跑豁免仍覆盖该档**（R-63 登记的「空转」现在**不是空转**了——它正是这次偶发的兜底）；**⇒ D20-11 的保守裁定在本轮产生了实际效用** |

### §12.5 分歧审计落档（**只读子代理 `5c987f01`，thorough**）

**结论：分歧 10 条 · 阻塞项 1 条。** 实验全在 `%TEMP%` 的**仓外副本**上做，**仓内零残留**（`git status --porcelain` 空、HEAD 仍 `66002f7`）。

| # | 级 | 发现（逐条带它自己的实测） | 父侧处置 |
|---|---|---|---|
| **1** | **🔴** | **AC-8 / A12 / FR-3③ / D20-8 / US-8 完全未实施**：`lib/index.mjs:712-715` 逐字仍是丢弃返回值 + `return send({ ok: true })`；全仓无一处碰 `DELETE /config` | **是父侧的错**（第 4 次收窄派发**漏派了 stage 3**，还把 `lib/index.mjs` 写进禁改面）⇒ **已派修复轮 stage 1** |
| **2** | 🟡 | FR-5 只做 2/4 档（`session-store.mjs` / `token-store.mjs` 零改动） | **已派修复轮 stage 2** |
| **3** | 🟡 | **AC-11 / A16 疗效门槛零证据**，而台账 §五已写「已根治」 | **父侧的两处处置**：① 台账措辞**降为「机制已根修，疗效门槛待补」**（见 §12.2 与台账）；② 门槛本身按 §10.3 AC-11 执行（×10/×20）——**在收口前跑并记录读数** |
| **4** | 🟡 | AC-13（交接页重启行）未做 | **父侧收口项**（T3，人工） |
| **5** | 🟡 | **§8.3 的 N4 负控不成立**：把写移回环外 ⇒ 红在 **A7+A8**（非 A9）；且 A9 的端到端触发器**新旧两版都零残留** ⇒ 「改前必留半个 tmp」**未成立** | **父侧改设计**（见 §8.3 的「★ 审计订正」） |
| **6** | 🟡 | **§8.3 的 N7 负控不成立**：删掉 TOCTOU 分支 ⇒ **零红** ⇒ FR-4 的 TOCTOU 修**零覆盖** | **Fixed**：修复轮补 **`A11b`（TOCTOU 真腿）**；**父侧仓外负控实证**：把 `if (e && e.code === "ENOENT") return true` 改成 `return false` ⇒ **`A11b` 恰红**（pass 10 / fail 1），**还原逐字节相等** |
| **7** | 🟡 | **锚 A2 未被锁定**：把原语调用换成正确 import 的裸 `renameSync` ⇒ 全绿 | **Fixed**：修复轮补 **`A2 源码级锁`**；**父侧仓外负控实证**：降级为裸 `renameSync` ⇒ **该条恰红**（另带 A11/A11b，pass 8 / fail 3），**还原逐字节相等** |
| **8** | 🔵 | §12.1 引用 `§12.3 / §12.4` 而当时**不存在** ⇒ 悬空指针 | **本节即其补写**（指针已可解析） |
| **9** | 🔵 | §12.2 止于第 3 次派发 | **本节上方已补第 4 次** |
| **10** | 🔵 | 台账 §五「已根治」落在**症状列**（A13 的字面谓词仍过） | **父侧改正**（移到处置列） |

**★ 审计实证为真、且**修正了父侧两处**的事实**：
- **D20-11 的理由成立**（★ 父侧裁定**不是猜的**）：在副本上删掉 §五 的 `R-13` 行 ⇒ `test/release-check.test.mjs` **pass 7 / fail 2**（3 条断言级：表非空 · 唯一失败项命中已知 flake · 首行登记被前者挡在后面），`node release-check.mjs v0.27.0` 报 **G6 FAIL** 并点名两项 ✓
- **§10.1 的 EOL 表逐字正确**：`lib/**` **19 CRLF / 11 LF**（30 档）· `test/*.test.mjs` **4 档 CRLF**；**且 CR 幻影复现**（`config-store` 371/363 与设计逐字相同；`dsh-home` 237/77 = 设计记 232/77 在第 2 次派发时点之后的 +5 版本）⇒ **审计自己先误判「幻影不复现」，随后用正确口径订正**
- **注入缝的 `unlink(from)` 保护真实**：提供 `write` 钩子 ⇒ `from` 被 unlink；**不提供** ⇒ `from` 存活、内容原样 ⇒ `clearUserConfig` 的**活配置文件**确实受保护 ✓
- **默认路径的真实墙钟**：真 fs（目标建成目录）⇒ `code=EPERM errno=-4048 attempts=8 **elapsedMs=311~312ms**` ⇒ **本档「最坏 ~210ms」是睡眠预算，真实墙钟约 1.5×** ⇒ **已在 §5.1 订正**
- **「新断言在旧实现下必红」这条对「新档」不成立**：旧 revision **不导出** `RENAME_BACKOFF_MS` ⇒ 新档整档 `SyntaxError` ⇒ **pass 0 / fail 1，零条具名用例红 = 设计 §8.3 自己定义的「假红」** ⇒ **已在 §8.3 订正**（对新档而言，可用的判据是「**逐条变异**」，审计已跑 N1⇒A1+A6+A6b · N2⇒A7+A9 · N3⇒A8 · N6⇒A10，**全为断言级真红**）
- **零改面 · 台账等值 · 合计数**：8 档强制零改面全空 · 六串 0/30 · `failStop(` 18 · §三 **21 档 == fs 21 档**（逐档计数零误差）· **494 == 494** ✓

**★ 审计自报「我无法验证的」8 项**（如实转登记）：真实杀软持有形态 · `R-13` 原发 errno · AC-11 的 ×10/×20 协议（它只跑 1 次全量）· 宿主日志面 · 重启后行为 · **A9「改前必留半个 tmp」的任意确定性触发器**（试了 4 个候选，Node v24.14.1 **无一**能让 `writeFileSync`「先建文件再抛」）· TOCTOU 真竞态窗口（单进程不可构造）· 真机持锁面。
