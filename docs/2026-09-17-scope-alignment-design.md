...
| **未声明档**（新增） | 现三条边界 + **`N == 项数`**，**再加三条宽松约束**：① **段内零 `·` 且 N≥2 ⇒ 跳过**（杀 CE-1/CE-2/CE-3 的零段形态）；② **只认两侧带空格的 ` · `**（杀 CE-5 的译名 `·`）；③ **尾随 `·`（末段为空）⇒ 跳过**（杀 CE-7 的折行形态） |
...
**`EXEMPT_BUDGET` 预算常数**：**全域有效豁免数 > 预算 ⇒ 红**（初值 **2**）。
...
| **A11** | 同上 | **exempt 四闸** | 四闸**各有一条负控**；**`EXEMPT_BUDGET` 超预算 ⇒ 红** |
...
| **AC-10** | **每条新腿 + 每条保守约束各有负控腿**（共 **12 条**），且各**必须红** | T1 | A8 · A11 | US-5 |
...
```mermaid
flowchart TD
  S1["**stage 1** FR-0 两处正确性修复（闭括号 + 围栏掩码）+ 其负控（改前红/改后绿）"] --> S2
  S2["**stage 2** FR-1 标记语法 + FR-3 双模式 + FR-4 归一化 + 负控"] --> S3
  S3["**stage 3** FR-2 事实 id 腿（分组 + 两条不变量）+ FR-5 CHANGELOG 两态 + 负控"] --> S4
  S4["**stage 4** FR-6 强制声明 + exempt 四闸 + 收口（本批两档加声明与 require-facts · 台账同步 · 全量 + A1–A13 + 变异自证）"]
  style S1 fill:#dff
  style S4 fill:#ffd
```
...
| stage | goal | 文件 | 检查 |
|---|---|---|---|
| **1** | FR-0a + FR-0b + 其负控 | `test/doc-hygiene.test.mjs` | `node --test test/doc-hygiene.test.mjs` |
| **2** | FR-1 + FR-3 + FR-4 + 负控 | `test/doc-hygiene.test.mjs` | 同上 |
| **3** | FR-2 + FR-5 + 负控 | `test/doc-hygiene.test.mjs` | 同上 |
| **4** | FR-6 + 收口（**★ 两档加声明与 `require-facts` 排进 stage 4，但 A12 的自证必须在 stage 4 完成**） | 全部 | `node --test` |
