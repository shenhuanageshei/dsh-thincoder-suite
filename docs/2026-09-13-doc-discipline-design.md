...
//     守护的是**旧切片**（无害，但它守护的**不是**它点名的锁）⇒ **批 12「夹具锁面收窄」已处置**
//     （批 12 交付：`docs/2026-09-15-lock-narrowing-design.md` §11.2 AC-15）。**处置内容如实记**：
//     批 12 **不**改这条断言本身（它守护旧切片、无害），改的是**别处**——① 真锁的区间下界未变，
//     内容由「函数体**全文含注释**」收窄为「**归一态（可执行行）**」（`normalizeExec`，`test/guard-e.test.mjs`）；
//     ② 描述本锁的**活在世**的「逐字节」措辞随之订正为「**可执行行逐字节**」。⇒ 本条要求的
//     「helper 不得落进 `WRITE_GATE_FIXTURE` 区间」**在收窄后仍然成立**（helper 落在归一态内即红）。
