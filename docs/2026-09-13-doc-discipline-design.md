...
| V5 / V9 / V15 | 新档自身不含六串；`codex-runner` **177/177**；台账 **19 档逐档相符，合计 453 == 实测** | ✅ |
...
**不挂 wrapper 的预检拒绝点**（工作未发生 ⇒ 自带补救指引、无菜单）：token 三态与续期腿（`:602`–`:657` 一带）· in-flight 单飞（`:478`）· `never issued`（`:582`）· `task` 缺失。
