...
- 日期：2026-09-01
- 状态：✅ 已实施（2026-09-02 v0.2 一期交付 + 评审收敛；§4 二期已被 settings-ui-design 取代）
...
> ⚠️ **Superseded（2026-09-02）**：本节与 §5.2 的 P 系列已被 `docs/2026-09-02-settings-ui-design.md` 取代（用户定案：自有 JSON config.json user 层 + imperative client + U 系列验收）。本节仅保留历史，实施以新文档为准。
...
二期验收（**P 系列，P1+，与一期 T 系列不冲突**）：P1 设置页渲染；P2 全局默认读写生效；P3 会话覆盖读写生效；P4 校验拦截（非法 provider/越界值内联报错）；P5 恢复会话默认——随二期实施细化。⚠️ **已被 U 系列取代**（见 docs/2026-09-02-settings-ui-design.md §5）。
